...
class BSHVariableDeclarator extends SimpleNode
{
	 ...
	 *            variable intializer children for the case where the array
	 *            initializer does not declare the type explicitly. e.g. int []
	 *            a = { 1, 2 }; typeNode may be null to indicate no type
	...
	public Object eval(BSHType typeNode, CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		if (jjtGetNumChildren() > 0)
		{
			...
			/*
			 * If we have type info and the child is an array initializer pass
			 * it along... Else use the default eval style. (This allows array
			 * initializer to handle the problem... allowing for future
			 * enhancements in loosening types there).
			 */
			if ((typeNode != null) && initializer instanceof BSHArrayInitializer)
			{
				value = ((BSHArrayInitializer) initializer).eval_2(typeNode.getBaseType(), typeNode.getArrayDims(), callstack, interpreter);
			}
			else
			{
