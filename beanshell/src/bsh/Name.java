 ...
 */
/*
 * <strong>Implementation notes</strong> <pre> Thread safety: all of the work
 ...
 * 
 * // does *not* work for( caller=this.caller; caller != null; caller =
 * caller.caller );
 * 
...
class Name implements Serializable
{
	...
	{
		return toObject_2(callstack, interpreter, false);
	}

	/**
	 * @see toObject()
	 * @param forceClass
	 *            if true then resolution will only produce a class. This is
	 *            necessary to disambiguate in cases where the grammar knows
	 *            that we want a class; where in general the var path may be
	 *            taken.
	 */
	synchronized public Object toObject_2(CallStack callstack, Interpreter interpreter, boolean forceClass) throws UtilEvalError
	{
		reset();
	...
	{
		/*
		 * Is it a simple variable name? Doing this first gives the correct Java
		 * precedence for vars vs. imported class names (at least in the simple
		 * case - see tests/precedence1.bsh). It should also speed things up a
		 * bit.
		 */
		...
		/*
		 * Is it a bsh script variable reference? If we're just starting the
		 * eval of name (no base object) or we're evaluating relative to a This
		 * type reference check.
		 */
		String varName = prefix_2(evalName, 1);
		...
		/*
		 * Is it a class name? If we're just starting eval of name try to make
		 * it, else fail.
		 */
		if (evalBaseObject == null)
		{
			...
			if (clas != null)
			{
				return completeRound(className, suffix_2(evalName, countParts(evalName) - i), new ClassIdentifier(clas));
			}
		...
		/*
		 * If we didn't find a class or variable name (or prefix) above there
		 * are two possibilities:
		 * 
		 * - If we are a simple name then we can pass as a void variable
		 * reference. - If we are compound then we must fail at this point.
		 */
		if (evalBaseObject == null)
		{
		...
		/*
		 * -------------------------------------------------------- After this
		 * point we're definitely evaluating relative to a base object.
		 * --------------------------------------------------------
		 */
		/*
		...
		if (evalBaseObject instanceof ClassIdentifier)
		{
			...
			String field = prefix_2(evalName, 1);
		...
		/*
		 * If we've fallen through here we are no longer resolving to a class
		 * type.
		 */
		if (forceClass)
		{
	 ...
	 * 
	 * @param callstack
	 *            may be null, but this is only legitimate in special cases
	 *            where we are sure resolution will not involve this.caller.
	 * @param namespace
	 *            the namespace of the this reference (should be the same as the
	 *            top of the stack?
	 */
	Object resolveThisFieldReference(CallStack callstack, NameSpace thisNameSpace, Interpreter interpreter, String varName, boolean specialFieldsVisible) throws UtilEvalError
	{
		...
		/*
		 * Some duplication for "super". See notes for "this" above If we're in
		 * an enclsing class instance and have a superclass instance our super
		 * is the superclass instance.
		 */
	 ...
	 *             (type of EvalError) on special case of ambiguous unqualified
	 *             name after super import.
	 */
	...
	*/
	...
	{
		String methodName = Name.suffix_2(value, 1);
	...
	private Object invokeLocalMethod(Interpreter interpreter, Object[] args, CallStack callstack, SimpleNode callerInfo) throws EvalError
	{
		...
		{
			throw e.toEvalError_2("Local method invocation", callerInfo, callstack);
		}
		// If defined, invoke it
		if (meth != null)
		{
			return meth.invoke_2(args, interpreter, callstack, callerInfo);
		}
		BshClassManager bcm = interpreter.getClassManager();
		...
		{
			throw e.toEvalError_2("Error loading command: ", callerInfo, callstack);
		}
		...
		if (commandObject == null)
		{
			...
			{
				return invokeMethod.invoke_2(new Object[] { commandName, args }, interpreter, callstack, callerInfo);
			}
		...
		{
			return ((BshMethod) commandObject).invoke_2(args, interpreter, callstack, callerInfo);
		}
		...
		{	
			...
			{
				throw e.toEvalError_2("Error invoking compiled command: ", callerInfo, callstack);
			}
		}
	 ...
	 * 
	 * private String getHelp( Class commandClass ) throws UtilEvalError { try {
	 * return (String)Reflect.invokeStaticMethod( null/bcm/, commandClass,
	 * "usage", null ); } catch( Exception e ) return "usage: "+name; } }
	 */
	...
	static String prefix(String value)
	{
		...
		return prefix_2(value, countParts(value) - 1);
	}

	static String prefix_2(String value, int parts)
	{
		if (parts < 1)
		{
			return null;
	...
	static String suffix(String name)
	{
		...
		return suffix_2(name, countParts(name) - 1);
	}

	public static String suffix_2(String value, int parts)
	{
		if (parts < 1)
		{
			return null;
