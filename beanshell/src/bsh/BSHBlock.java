...
class BSHBlock extends SimpleNode
{
	...
	{
		return eval_2(callstack, interpreter, false);
	}

	/**
	 * @param overrideNamespace
	 *            if set to true the block will be executed in the current
	 *            namespace (not a subordinate one).
	 *            <p>
	 *            If true *no* new BlockNamespace will be swapped onto the stack
	 *            and the eval will happen in the current top namespace. This is
	 *            used by BshMethod, TryStatement, etc. which must intialize the
	 *            block first and also for those that perform multiple passes in
	 *            the same block.
	 */
	public Object eval_2(CallStack callstack, Interpreter interpreter, boolean overrideNamespace) throws EvalError
	{
	...
	Object evalBlock(CallStack callstack, Interpreter interpreter, boolean overrideNamespace, NodeFilter nodeFilter) throws EvalError
	{
		...
		{
			/*
			 * Evaluate block in two passes: First do class declarations then do
			 * everything else.
			 */
