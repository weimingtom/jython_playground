 ...
 * 
 * Note on version dependency: This base class is JDK 1.1 compatible, however we
 * are forced to use weak references in the full featured implementation (the
 * optional bsh.classpath package) to accomodate all of the fleeting namespace
 * listeners as they fall out of scope. (NameSpaces must be informed if the
 * class space changes so that they can un-cache names). <p>
 * 
 * Perhaps a simpler idea would be to have entities that reference cached types
 * always perform a light weight check with a counter / reference value and use
 * that to detect changes in the namespace. This puts the burden on the consumer
 * to check at appropriate times, but could eliminate the need for the listener
 * system in many places and the necessity of weak references in this package.
 * <p>
 */
public class BshClassManager
{
	 ...
	 * 
	 * @param value
	 *            if value is non-null, cache the class if value is null, set
	 ...
	 * bytecode when a scripted class of the same name is encountered. When such
	 * a class is defined in the script it will cause the associated existing
	 * class implementation to be initialized (with the static initializer
	 * field). This is utilized by the persistent class generator to allow a
	 * generated class to bootstrap an interpreter and rendesvous with its
	...
	public void addListener(BshClassManagerListener l)
	{
	}

	public void removeListener(BshClassManagerListener l)
	{
	}
	 ...
	 * process of defining two classes in different packages with the same base
	 * name. To remove this limitation requires that we work through namespace
	 * imports in an analogous (or using the same path) as regular class import
	 * resolution. This workaround should handle most cases so we'll try it for
	 * now.
	 */
	protected void definingClass(String className)
	{
		String baseName = Name.suffix_2(className, 1);
	...
	public Class defineClass(String name, byte[] code)
	{
		...
		/*
		 * Old implementation injected classes into the parent classloader. This
		 * was incorrect behavior for several reasons. The biggest problem is
		 * that classes could therefore only be defined once across all
		 * executions of the script...
		 * 
		 * ClassLoader cl = this.getClass().getClassLoader(); Class clas; try {
		 * clas = (Class)Reflect.invokeObjectMethod( cl, "defineClass", new
		 * Object [] { name, code, new Primitive( (int)0 )/offset/, new
		 * Primitive( code.length )/len/ }, (Interpreter)null, (CallStack)null,
		 * (SimpleNode)null ); } catch ( Exception e ) { e.printStackTrace();
