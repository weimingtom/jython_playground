...
class BSHPrimarySuffix extends SimpleNode
{
	...
	/*
	 * Perform a suffix operation on the given object and return the new value.
	 * <p>
	 * 
	 ...
	 * value object or LHS (as determined by toLHS). <p>
	 * 
	...
	public Object doSuffix(Object obj, boolean toLHS, CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		/*
		 * Evaluate our prefix if it needs evaluating first. If this is the
		 * first evaluation our prefix mayb be a Node (directly from the
		 * PrimaryPrefix) - eval() it to an object. If it's an LHS, resolve to a
		 * value.
		 * 
		 ...
		 * make a static method nodeToObject() or something. The point is that
		 * we can't just eval() - we need to direct the evaluation to the
		 * context sensitive type of result; namely object, class, etc.
		 */
	...
	/*
	 * Field access, .length on array, or a method invocation Must handle toLHS
	 * case for each.
	 */
	...
	*/
	static int getIndexAux(Object obj, CallStack callstack, Interpreter interpreter, SimpleNode callerInfo) throws EvalError
	{
		...
		catch (UtilEvalError e)
		{
			...
			throw e.toEvalError_2("Arrays may only be indexed by integer types.", callerInfo, callstack);
		}
		return index;
	}
	...
	private Object doProperty(boolean toLHS, Object obj, CallStack callstack, Interpreter interpreter) throws EvalError
	{
		...
		{
			throw e.toEvalError_2("Property: " + value, this, callstack);
		} 
		catch (ReflectError e)
		{
