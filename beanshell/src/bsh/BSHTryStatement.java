...
class BSHTryStatement extends SimpleNode
{
	...
	public Object eval(CallStack callstack, Interpreter interpreter) throws EvalError
	{
		 ...
		 * TargetErrors generated by the script. We save the callstack depth and
		 * if an exception is thrown we pop back to that depth before contiuing.
		 * The exception short circuited any intervening method context pops.
		 * 
		 * Note: we the stack info... what do we do with it? append to exception
		 * message?
		 */
		...
		if (thrown != null)
		{
			...
			for (i = 0; i < n; i++)
			{
				...
				{	
					...
					{
						/*
						 * Catch the mismatch and continue to try the next Note:
						 * this is innefficient, should have an
						 * isAssignableFrom() that doesn't throw // TODO: we do
						 * now have a way to test assignment // in castObject(),
						 * use it?
						 */
						continue;
					}
				...
				try
				{
					...
					else
					{
						...
						cbNameSpace.setTypedVariable_2(fp.name, fp.type, thrown, new Modifiers()/* none */);
					}
				} 
				catch (UtilEvalError e)
				{
