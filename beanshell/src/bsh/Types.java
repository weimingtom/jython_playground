...
class Types
{
	 ...
	 * normally happen on assignment. A CAST performs numeric conversions to
	 * smaller types (as in an explicit Java cast) and things allowed only in
	 * variable and array declarations (e.g. byte b = 42;)
	 */
	...
	/**
	 * Are the two signatures exactly equal? This is checked for a special case
	...
	{
		/*
		 * Assignment to loose type, defer to bsh extensions Note: we could
		 * shortcut this here: if ( lhsType == null ) return true; rather than
		 * forcing another round. It's not strictly a Java issue, so does it
		 * belong here?
		 */
		if (lhsType == null)
		{
	...
	static boolean isBshAssignable(Class toType, Class fromType)
	{
		try
		{
			return castObject(toType, fromType, null/* fromValue */, ASSIGNMENT, true/* checkOnly */) == VALID_CAST;
		} 
	 ...
	 * 
	 * @param toType
	 *            the class type of the cast result, which may include primitive
	 *            types, e.g. Byte.TYPE
	 * @param fromValue
	 *            an Object or bsh.Primitive primitive value (including
	 ...
	 * @param toType
	 *            the class type of the cast result, which may include primitive
	 *            types, e.g. Byte.TYPE. toType may be null to indicate a loose
	 *            type assignment (which matches any fromType).
	 * @param fromType
	 *            is the class type of the value to be cast including java
	 *            primitive TYPE classes for primitives. If fromValue is (or
	 *            would be) Primitive.NULL then fromType should be null.
	 * @param fromValue
	 *            an Object or bsh.Primitive primitive value (including
	 *            Primitive.NULL or Primitive.VOID )
	 * @param checkOnly
	 *            If checkOnly is true then fromValue must be null. FromType is
	 *            checked for the cast to toType... If checkOnly is false then
	 *            fromValue must be non-null (Primitive.NULL is ok) and the
	 *            actual cast is performed.
	 * @throws UtilEvalError
	 *             on invalid assignment (when operation is assignment ).
	 * @throws UtilTargetError
	 *             wrapping ClassCastException on cast error (when operation is
	 *             cast)
	 * @param operation
	...
	{
		/*
		 * Lots of preconditions checked here... Once things are running
		 * smoothly we might comment these out (That's what assertions are for).
		 */
		...
		if (toType.isInterface() && bsh.This.class.isAssignableFrom(fromType) && Capabilities.canGenerateInterfaces())
		{
