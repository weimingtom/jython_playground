...
public class RubyString extends RubyObject
{
	...
	/**
	 * Create a new String which uses the same Ruby runtime and the same class
	 * like this String.
	 * 
	...
	public IRubyObject aref(IRubyObject[] args)
	{
		if (checkArgumentCount(args, 1, 2) == 2)
		{
			if (args[0] instanceof RubyRegexp)
			{
				...
				return match.isNil() ? getRuntime().getNil() : ((RubyMatchData) match).group(args[1].convertToInteger().getLongValue());
			}
		...
		if (args[0] instanceof RubyRegexp)
		{
			return RubyRegexp.regexpValue(args[0]).search(toString(), 0) >= 0 ? RubyRegexp.last_match(getRuntime().getCurrentContext().getBackref()) : getRuntime().getNil();
		} else if (args[0] instanceof RubyString)
		{
			return toString().indexOf(stringValue(args[0]).toString()) != -1 ? args[0] : getRuntime().getNil();
		...
		return idx < 0 || idx >= getValue().length() ? getRuntime().getNil() : getRuntime().newFixnum(getValue().charAt(idx));
	}
	...
	{
		long base = checkArgumentCount(args, 0, 1) == 0 ? 10 : ((RubyInteger) args[0].convertType(RubyInteger.class, "Integer", "to_i")).getLongValue();
	 ...
	 * rb_str_chomp_bang
	 * 
	 * In the common case, removes CR and LF characters in various ways
	 * depending on the value of the optional args[0]. If args.length==0 removes
	 * one instance of CR, CRLF or LF from the end of the string. If
	 * args.length>0 and args[0] is "\n" then same behaviour as args.length==0 .
	 * If args.length>0 and args[0] is "" then removes trailing multiple LF or
	 * CRLF (but no CRs at all(!)).
	 * 
	 * @param args
	...
	public IRubyObject chomp_bang(IRubyObject[] args)
	{
		...
		// Separator (a.k.a. $/) can be overriden by the -0 (zero) command line
		// option
		String separator = (args.length == 0) ? getRuntime().getGlobalVariables().get("$/").asSymbol() : args[0].asSymbol();
