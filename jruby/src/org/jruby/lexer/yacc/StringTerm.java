...
public class StringTerm extends StrTerm
{
	...
	public int parseString(final RubyYaccLexer lexer, LexerSource src)
	{
		...
		c = src.read();
		if ((func & RubyYaccLexer.STR_FUNC_QWORDS) != 0 && Character.isWhitespace(c))
		{
			do
			{
				c = src.read();
		...
		if (c == term && nest == 0)
		{
			...
			{
				lexer.yaccValue = new RegexpNode(src.getPosition(), buffer.toString(), parseRegexpFlags(src));
				...
				switch (c)
				{
					...
					default:
						...
						} else if ((func & RubyYaccLexer.STR_FUNC_EXPAND) != 0)
						{
							...
							c = src.readEscape();
						} else if ((func & RubyYaccLexer.STR_FUNC_QWORDS) != 0 && Character.isWhitespace(c))
						{
