...
/**
 * This class is what feeds the lexer. It is primarily a wrapper around a Reader
 * that can unread() data back onto the source. Originally, I thought about
 * using the PushBackReader to handle read/unread, but I realized that some
 * extremely pathological case could overflow the pushback buffer. Better safe
 * than sorry. I could have combined this implementation with a PushbackBuffer,
...
public class LexerSource
{
	...
	/**
	 * Pushes char back onto this source. Note, this also allows us to push
	 * whatever is passes back into the source.
	 * 
	 * @param c
	...
	/**
	 * Convenience method to hide exception. If we do hit an exception we will
	 * pretend we EOF'd.
	 * 
	...
	private char wrappedRead()
	{
		try
		{
			...
			if (c == '\r')
			{
				...
				{
					// Position within source must reflect the actual offset and
					// column. Since
					// we ate an extra character here (this accounting is
					// normally done in read
