...
public class JavaField extends JavaAccessibleObject
{
	...
	public JavaObject set_value(IRubyObject object, IRubyObject value)
	{
		...
		try
		{
			Object convertedValue = JavaUtil.convertArgument(((JavaObject) value).getValue(), field.getType());
		...
		{
			throw getRuntime().newTypeError("wrong type for " + field.getType().getName() + ": " + ((JavaObject) value).getValue().getClass().getName());
		}
