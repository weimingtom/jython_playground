...
class OSEnvironmentReaderFromRuntimeExec implements IOSEnvironmentReader
{
	...
	private String getEnvCommand()
	{
		...
		// TODO merge this logic into RubyKernel.runInShell? / possible conflict
		// if jruby.shell System property is set

		if (osname.indexOf("windows 9") > -1)
		{
			command = "command.com /c set";
		} else if ((osname.indexOf("nt") > -1) || (osname.indexOf("windows 20") > -1) || (osname.indexOf("windows xp") > -1))
		{
	...
	public Map getVariables(IRuby runtime)
	{
		...
		try
		{
			RubyKernel.runInShell(runtime, new IRubyObject[] { RubyString.newString(runtime, getEnvCommand()) }, output);
