...
public class RubyFloat extends RubyNumeric
{
	...
	public IRubyObject op_div(IRubyObject other)
	{
		...
		{
			return RubyFloat.newFloat(getRuntime(), getDoubleValue() / ((RubyNumeric) other).getDoubleValue());
		}
	...
	// TODO: Anders double-dispatch here does not seem like it has much benefit
	// when we need
	...
	public IRubyObject op_pow(IRubyObject other)
	{
		if (other instanceof RubyNumeric)
		{
			return RubyFloat.newFloat(getRuntime(), Math.pow(getDoubleValue(), ((RubyNumeric) other).getDoubleValue()));
		}
