...
public class RubyRange extends RubyObject
{
	 ...
	 * and length. If either of the range's endpoints is negative, it is added
	 * to the <code>limit</code> parameter in an attempt to arrive at a position
	 * <i>p</i> such that <i>0&nbsp;&lt;=&nbsp;p&nbsp;&lt;=&nbsp;limit</i>. If
	 * <code>truncate</code> is true, the result will be adjusted, if possible,
	 * so that <i>begin&nbsp;+&nbsp;length&nbsp;&lt;=&nbsp;limit</i>. If
	 * <code>strict</code> is true, an exception will be raised if the range
	 * can't be converted as described above; otherwise it just returns
	 * <b>null</b>.
	 * 
	 * @param limit
	 *            the size of the object (e.g., a String or Array) that this
	 *            range is being evaluated against.
	 * @param truncate
	 *            if true, result must fit within the range <i>(0..limit)</i>.
	 * @param isStrict
	 *            if true, raises an exception if the range can't be converted.
	 * @return a two-element array representing a start value and a length, or
	 *         <b>null</b> if the conversion failed.
	 */
	...
	public RubyFixnum length()
	{
		...
		{ // Support length for arbitrary classes
	...
	public IRubyObject equal(IRubyObject obj)
	{
		...
		boolean result = begin.equals(otherRange.begin) && end.equals(otherRange.end) && isExclusive == otherRange.isExclusive;
	...
	public RubyBoolean op_eqq(IRubyObject obj)
	{
		...
		{// FIXME should be anything that doesn't responds to <=??
			return getRuntime().getFalse();
		}
	...
	public RubyBoolean include_p(IRubyObject obj)
	{
		...
		return f.getRuntime().newBoolean(f != first().callMethod("<=", arg) && f != last().callMethod(compareMethod, arg));
	}
}
