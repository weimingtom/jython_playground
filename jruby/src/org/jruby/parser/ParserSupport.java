...
public class ParserSupport
{
	...
	public Node appendPrintToBlock(Node block)
	{
		...
		return appendToBlock(block, new FCallNode(position, "print", new ArrayNode(position).add(new GlobalVarNode(position, "$_"))));
	}
	...
	public Node appendWhileLoopToBlock(Node block, boolean chop, boolean split)
	{
		...
		if (split)
		{
			block = appendToBlock(new GlobalAsgnNode(position, "$F", new CallNode(position, new GlobalVarNode(position, "$_"), "split", null)), block);
		}
		if (chop)
		{
	...
	public Node gettable(String id, ISourcePosition position)
	{
		 ...
		 * TODO: add __FILE__ and __LINE__ support? else if (id == k__FILE__) {
		 * return NEW_STR(rb_str_new2(ruby_sourcefile)); } else if (id ==
		 * k__LINE__) { return NEW_LIT(INT2FIX(ruby_sourceline)); }
		 */
		else if (IdUtil.isLocal(id))
		{
			...
			return new VCallNode(position, id); // RubyMethod call without
												// arguments.
	...
	public Node assignable(ISourcePosition position, Object id, Node value)
	{
		...
		/*
		 * else if (id == k__FILE__) { yyerror("Can't assign to __FILE__"); }
		 * else if (id == k__LINE__) { yyerror("Can't assign to __LINE__"); }
		 */else
		{
			String name = null;
			...
			if (IdUtil.isLocal(name))
			{
				...
				/*
				 * if (rb_dvar_curr(id)) { return NEW_DASGN_CURR(id, value); }
				 * else
				 */
				BlockNamesElement blockNames = (BlockNamesElement) blockNamesStack.peek();
				LocalNamesElement localNames = (LocalNamesElement) localNamesStack.peek();
				...
				{
					return new LocalAsgnNode(position, name, localNames.getLocalIndex(name), value);
				} else
				{
					blockNames.add(name);
	...
	/**
	 * Check all nodes but the last one in a BlockNode for useless (void
	 * context) statements.
	 * 
	 * @param blockNode
	 *            to be checked.
	...
	private Node cond0(Node node)
	{
		...
		{
			return new FlipNode(node.getPosition(), getFlipConditionNode(((DotNode) node).getBeginNode()), getFlipConditionNode(((DotNode) node).getEndNode()), ((DotNode) node).isExclusive(), ((LocalNamesElement) localNamesStack.peek()).registerLocal(null));
	...
	{
		/*
		 * Node node = ((BlockPassNode) args).getArgsNode(); IListNode argsNode
		 * = null;
		 * 
		 * if (node instanceof IListNode) { argsNode = (IListNode) node; } else
		 * if (node != null){ argsNode = new
		 * ArrayNode(node.getPosition()).add(node); }
		 * 
		 * ((BlockPassNode) args).setIterNode(new
		 * CallNode(receiverNode.getPosition(), receiverNode, name, argsNode));
		 * return args;
		 */
	...
	public void updateTopLocalVariables()
	{
		...
		result.setBlockVariables(localNames.isInBlock() ? ((BlockNamesElement) blockNamesStack.peek()).getNames() : null);
	 ...
	 * 
	 * @param inDefined
	 *            New value of property inDefined.
	 */
	...
	public Node literal_concat(ISourcePosition position, Node head, Node tail)
	{
		...
		if (tail instanceof StrNode)
		{
			...
			{
				head = new StrNode(union(head, (Node) tail), ((StrNode) head).getValue() + ((StrNode) tail).getValue());
			} else
			{
