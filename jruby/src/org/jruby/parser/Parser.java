...
public class Parser
{
	...
	{
		return parse(file, content, new RubyParserConfiguration(), runtime.getObject().getCRef());
	}
	...
	private Node parse(String file, Reader content, RubyParserConfiguration config, SinglyLinkedList cref)
	{
		config.setLocalVariables(runtime.getCurrentContext().getCurrentScope().getLocalNames());

		// search for an ITER_CUR; this lets us know we're parsing from within a
		// block and should bring dvars along
