...
public class RubyIO extends RubyObject
{
	 ...
	 * concurrency issues happen when the underlying implementation commits
	 * data. So:
	 * 
	 * f = File.new("some file", "w") f.puts("heh") g = IO.new(f.fileno)
	 * g.puts("hoh") ... more operations of g and f ...
	 * 
	 * Will generate a mess in "some file". The problem is that most operations
	 * are buffered. When those buffers flush and get written to the physical
	 * file depends on the implementation (semantically I would think that it
	 * should be last op wins -- but it isn't). I doubt java could mimic ruby in
	 * this way. I also doubt many people are taking advantage of this. How
	 * about syswrite/sysread though? I think the fact that sysread/syswrite are
	 * defined to be a low-level system calls, allows implementations to be
	 * somewhat different?
	 * 
	 * 2. In the case of: f = File.new("some file", "w") f.puts("heh") print
	 * f.pos g = IO.new(f.fileno) print g.pos Both printed positions will be the
	 * same. But: f = File.new("some file", "w") f.puts("heh") g =
	 * IO.new(f.fileno) print f.pos, g.pos won't be the same position. Seem
	 * peculiar enough not to touch (this involves pos() actually causing a
	 * seek?)
	 * 
	 * 3. All IO objects reference a IOHandler. If multiple IO objects have the
	 * same fileno, then they also share the same IOHandler. It is possible that
	 * some IO objects may share the same IOHandler but not have the same
	 * permissions. However, all subsequent IO objects created after the first
	 * must be a subset of the original IO Object (see below for an example).
	 * 
	 * The idea that two or more IO objects can have different access modes
	 * means that IO objects must keep track of their own permissions. In
	 * addition the IOHandler itself must know what access modes it has.
	 * 
	 * The above sharing situation only occurs in a situation like: f =
	 * File.new("some file", "r+") g = IO.new(f.fileno, "r") Where g has reduced
	 * (subset) permissions.
	 * 
	...
	// FIXME can't use static; would interfere with other runtimes in the same
	// JVM
	...
	/*
	 * What the IO object "thinks" it can do. If two IO objects share the same
	 * fileno (IOHandler), then it is possible for one to pull the rug out from
	 * the other. This will make the second object still "think" that the file
	 * is open. Secondly, if two IO objects share the same fileno, but the
	 * second one only has a subset of the access permissions, then it will
	 * "think" that it cannot do certain operations.
	 */
	...
	public IRubyObject initialize(IRubyObject[] args)
	{
		...
		} else
		{
			...
			modes = mode == null ? handler.getModes() : new IOModes(getRuntime(), mode);
	...
	// This appears to be some windows-only mode. On a java platform this is a
	// no-op
	...
	/**
	 * <p>
	 * Return the process id (pid) of the process this IO object spawned. If no
	 * process exists (popen was not called), then nil is returned. This is not
	 * how it appears to be defined but ruby 1.8 works this way.
	 * </p>
	 * 
	...
	/**
	 * <p>
	 * Closes all open resources for the IO. It also removes it from our magical
	 * all open file descriptor pool.
	 * </p>
	 * 
	 * @return The IO.
	 */
	...
	public IRubyObject fcntl(IRubyObject cmd, IRubyObject arg) throws IOException
	{
		...
		// FIXME: Arg may also be true, false, and nil and still be valid.
		// Strangely enough,
	...
	{
		for (IRubyObject line = internalGets(args); !line.isNil(); line = internalGets(args))
		{
	...
	public RubyArray readlines(IRubyObject[] args)
	{
		...
		if (args.length > 0)
		{
			...
			{
				throw getRuntime().newTypeError(args[0], getRuntime().getClass("String"));
			}
