...
/**
 * Class used to launch the interpreter. This is the main class as defined in
 * the jruby.mf manifest. It is very basic and does not support yet the same
 * array of switches as the C interpreter. Usage: java -jar jruby.jar [switches]
 * [rubyfile.rb] [arguments] -e 'command' one line of script. Several -e's
 * allowed. Omit [programfile]
 * 
 * @author jpetersen
 */
public class Main
{
	...
	private void initializeRuntime(final IRuby runtime, String filename)
	{
		...
		runtime.getObject().setConstant("$VERBOSE", commandline.isVerbose() ? runtime.getTrue() : runtime.getNil());
