...
public class RubyKernel
{
	public static RubyModule createKernelModule(IRuby runtime)
	{
		...
		// TODO: Add deprecation to Kernel#iterator? (maybe formal deprecation
		// mech.)
		...
		// Note: method_missing is documented as being in Object, but ruby
		// appears to stick it in Kernel.
	...
	public static IRubyObject sleep(IRubyObject recv, IRubyObject seconds)
	{
		...
		return recv.getRuntime().newFixnum(Math.round((System.currentTimeMillis() - startTime) / 1000.0));
	}

	// FIXME: Add at_exit and finalizers to exit, then make exit_bang not call
	// those.
	...
	/**
	 * Require. MRI allows to require ever .rb files or ruby extension dll (.so
	 * or .dll depending on system). we allow requiring either .rb files or
	 * jars.
	 * 
	 * @param recv
	 *            ruby object used to call require (any object will do and it
	...
	/**
	 * For the first full token on the command, most likely the actual
	 * executable to run, replace all dir separators with that which is
	 * appropriate for the current platform. Return the new with this executable
	 * string at the beginning.
	 * 
	 * @param command
	 *            The all-forward-slashes command to be "fixed"
	...
	private static String repairDirSeps(String command)
	{
		...
		// Matcher.replaceAll treats backslashes in the replacement string as
		// escaped characters
	...
	public static int runInShell(IRuby runtime, IRubyObject[] rawArgs, OutputStream output)
	{
		try
		{
			// startup scripts set jruby.shell to /bin/sh for Unix, cmd.exe for
			// Windows
			...
			if (aProcess != null)
			{
				...
				// whether the last line in a process ended with a newline or
				// not.
				int b;
	...
	public static RubyInteger srand(IRubyObject recv, IRubyObject[] args)
	{
		...
		} else
		{
			...
			runtime.setRandomSeed(System.currentTimeMillis() ^ recv.hashCode() ^ runtime.incrementRandomSeedSequence() ^ runtime.getRandom().nextInt(Math.abs((int) runtime.getRandomSeed())));
		}
