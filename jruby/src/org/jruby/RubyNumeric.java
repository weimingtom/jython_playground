...
public class RubyNumeric extends RubyObject
{
	 ...
	 * Parsing starts at the beginning of the string (after leading and trailing
	 * whitespace have been removed), and stops at the end or at the first
	 * character that can't be part of an integer. Leading signs are allowed. If
	 * <code>base</code> is zero, strings that begin with '0[xX]', '0[bB]', or
	 * '0' (optionally preceded by a sign) will be treated as hex, binary, or
	 * octal numbers, respectively. If a non-zero base is given, only the prefix
	 * (if any) that is appropriate to that base will be parsed correctly. For
	 * example, if the base is zero or 16, the string "0xff" will be converted
	 * to 256, but if the base is 10, it will come out as zero, since 'x' is not
	 * a valid decimal digit. If the string fails to parse as a number, zero is
	 * returned.
	 * 
	 * @param runtime
	 *            the ruby runtime
	 ...
	 * @param base
	 *            the expected base of the number (2, 8, 10 or 16), or 0 if the
	 *            method should determine the base automatically (defaults to
	 *            10).
	 * @return a RubyFixnum or (if necessary) a RubyBignum representing the
	 *         result of the conversion, which will be zero if the conversion
	 *         failed.
	 */
	 ...
	 * leading and trailing whitespace have been removed), and stops at the end
	 * or at the first character that can't be part of a number. If the string
	 * fails to parse as a number, 0.0 is returned.
	 * 
	 * @param runtime
	 *            the ruby runtime
	 * @param arg
	 *            the string to be converted
	 * @return a RubyFloat representing the result of the conversion, which will
	 *         be 0.0 if the conversion failed.
	 */
