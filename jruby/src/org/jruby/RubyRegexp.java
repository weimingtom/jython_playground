...
public class RubyRegexp extends RubyObject implements ReOptions
{
	...
	/**
	 * Class which represents the multibyte character set code. (should be an
	 * enum in Java 5.0).
	 * 
	...
	public IRubyObject equal(IRubyObject other)
	{
		...
		checkInitialized();
		if (!(re.pattern.pattern().equals(pattern.pattern()) && re.pattern.flags() == pattern.flags()))
		{
			return getRuntime().getFalse();
		}
	...
	public IRubyObject match(IRubyObject target)
	{
		...
		return result < 0 ? getRuntime().getNil() : getRuntime().newFixnum(result);
	}
	 ...
	 * 
	 * @param buffer
	 *            the default buffer for the output
	 * @param off
	 *            temporary buffer for the off flags
	 * @param flag
	 *            a Pattern flag
	 * @param c
	 *            the char which represents the flag
	 */
