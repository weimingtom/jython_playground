...
public class RubyDir extends RubyObject
{
	...
	public static RubyClass createDirClass(IRuby runtime)
	{
		...
		// dirClass.defineSingletonMethod("delete",
		// callbackFactory.getSingletonMethod(RubyDir.class, "delete",
		// RubyString.class));
	 ...
	 * contents of the directory at creation time, so changes to the contents of
	 * the directory will not be reflected during the lifetime of the
	 * <code>Dir</code> object returned, so a new <code>Dir</code> instance must
	 * be created to reflect changes to the underlying file system.
	 */
	...
	public static IRubyObject chdir(IRubyObject recv, IRubyObject[] args)
	{
		...
		RubyString path = args.length == 1 ? (RubyString) args[0].convertToString() : getHomeDirectoryPath(recv);
	...
	public static IRubyObject foreach(IRubyObject recv, RubyString path)
	{
		...
		RubyDir dir = (RubyDir) newInstance(recv.getRuntime().getClass("Dir"), new IRubyObject[] { path });
	...
	{
		RubyDir directory = (RubyDir) newInstance(recv.getRuntime().getClass("Dir"), new IRubyObject[] { path });
	 ...
	 * 
	 * @param path
	 *            path for which to return the <code>File</code> object.
	 * @param mustExist
	...
	/*
	 * Poor mans find home directory. I am not sure how windows ruby behaves
	 * with '~foo', but this mostly will work on any unix/linux/cygwin system.
	 * When someone wants to extend this to include the windows way, we should
	 * consider moving this to an external ruby file.
	 */
	public static IRubyObject getHomeDirectoryPath(IRubyObject recv, String user)
	{
		// TODO: Having a return where I set user inside readlines created a
		// JumpException. It seems that
