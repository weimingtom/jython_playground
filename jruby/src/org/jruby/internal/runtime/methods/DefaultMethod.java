...
public final class DefaultMethod extends AbstractMethod
{
	...
	public DefaultMethod(RubyModule implementationClass, ScopeNode body, ArgsNode argsNode, Visibility visibility, SinglyLinkedList cref)
	{
	...
	/**
	 * @see AbstractCallable#call(IRuby, IRubyObject, String, IRubyObject[],
	 *      boolean)
	 */
	...
	private void prepareArguments(IRuby runtime, Scope scope, IRubyObject receiver, IRubyObject[] args)
	{
		...
		if (scope.hasLocalVariables())
		{
			...
			if (argsNode.getOptArgs() != null)
			{
				...
				{
					// new AssignmentVisitor(new EvaluationState(runtime,
					// receiver)).assign((Node)iter.next(), args[i], true);
				...
				while (iter.hasNext())
				{
					// new EvaluationState(runtime,
					// receiver).begin((Node)iter.next());
					// EvaluateVisitor.getInstance().eval(receiver.getRuntime(),
					// receiver, (Node)iter.next());
					// in-frame EvalState should already have receiver set as
					// self, continue to use it
	...
	private void traceCall(IRuby runtime, IRubyObject receiver, String name)
	{
		...
		ISourcePosition position = body.getBodyNode() != null ? body.getBodyNode().getPosition() : body.getPosition();
