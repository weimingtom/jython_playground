...
public class RubyModule extends RubyObject
{
	...
	// Containing class...The parent of Object is null. Object should always be
	// last in chain.
	...
	// All methods and all CACHED methods for the module. The cached methods
	// will be removed
	// when appropriate (e.g. when method is removed by source class or a new
	// method is added
	 ...
	 * included modules, and supermodules.
	 * 
	...
	{
		IRubyObject result = setInstanceVariable(name, value, "Insecure: can't set constant", "class/module");
	...
	public synchronized void includeModule(IRubyObject arg)
	{
		...
		// Invalidate cache for all methods in the new included module in case a
		// base class method
		...
		// cnutter: removed iterative inclusion of module's superclasses,
		// because it included them in reverse order
	...
	{
		// Included modules modify the original 'included' modules class. Since
		// multiple
		// classes can include the same module, we cannot cache in the original
		// included module.
		if (!isIncluded())
		{
	...
	public void addMethod(String name, ICallable method)
	{
		...
		{
			// If we add a method which already is cached in this class, then we
			// should update the
	...
	public ICallable searchMethod(String name)
	{
		...
		{
			// included modules use delegates methods for we need to synchronize
			// on result of getMethods
			synchronized (searchModule.getMethods())
			{
				// See if current class has method or if it has been cached here
				// already
		...
		retry: while (true)
		{
			while (p != null)
			{
				...
				if (constant != null)
				{
					...
					{
						getRuntime().getWarnings().warn("toplevel constant " + name + " referenced by " + getName() + "::" + name);
					}
	...
	public synchronized void defineAlias(String name, String oldName)
	{
		...
		if (method.isUndefined())
		{
			...
			if (method.isUndefined())
			{
				throw getRuntime().newNameError("undefined method `" + oldName + "' for " + (isModule() ? "module" : "class") + " `" + getName() + "'");
			}
		}
	...
	{
		// Modules do not directly define Object as their superClass even though
		// in theory they
		// should. The C version of Ruby may also do this (special checks in
		// rb_alias for Module
		// makes me think this).
	...
	public boolean isKindOfModule(RubyModule type)
	{
		for (RubyModule p = this; p != null; p = p.getSuperClass())
		{
			// FIXME: this equality check is totally lame; isKindOf should be
			// enough
	...
	private RubyArray instance_methods(IRubyObject[] args, final Visibility visibility)
	{
		...
		for (RubyModule type = this; type != null; type = type.getSuperClass())
		{
			...
			for (Iterator iter = type.getMethods().entrySet().iterator(); iter.hasNext();)
			{
				...
				if (method.getImplementationClass() == realType && method.getVisibility().is(visibility) && undefinedMethods.get(methodName) == null)
				{
