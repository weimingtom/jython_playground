...
public class RubyFileStat extends RubyObject
{
	...
	public static RubyClass createFileStatClass(IRuby runtime)
	{
		...
		// fileStatClass.defineMethod("grpowned?",
		// callbackFactory.getMethod(""));
		...
		// fileStatClass.defineMethod("rdev_major",
		// callbackFactory.getMethod(""));
		// fileStatClass.defineMethod("rdev_minor",
		// callbackFactory.getMethod(""));
		fileStatClass.defineMethod("readable?", callbackFactory.getMethod("readable_p"));
		// fileStatClass.defineMethod("readable_real?",
		// callbackFactory.getMethod(""));
		...
		// fileStatClass.defineMethod("writable_real?",
		// callbackFactory.getMethod(""));
	...
	protected RubyFileStat(IRuby runtime, JRubyFile file)
	{
		...
		// implementation to lowest common denominator...Windows has no file
		// mode, but C ruby returns either 0100444 or 0100666
		...
		// We cannot determine this in Java, so we will always return false
	...
	// Limitation: We have no pure-java way of getting inode. webrick needs this
	// defined to work.
