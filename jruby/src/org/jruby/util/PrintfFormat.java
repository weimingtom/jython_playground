...
/**
 * PrintfFormat is a Java implementation of the <code>sprintf</code> utility. To
 * use it, you pass the string containing the format specifier(s) to the
 * constructor, then call the appropriate form of the <code>sprintf</code>
 * method. To pass more than one argument, convert any primitives to their
 * wrapper types and place all the arguments in an array of type
 * <code>Object</code>.
 *<p>
 * PrintfFormat supports all the standard flags (<code>[-+ #0]</code>), as well
 * as the <code>'</code> (apostrophe) flag, which causes decimal integers to be grouped
 * in a locale-dependent way (e.g, in English locales, groups of three digits
 * separated by commas). The integer modifiers (<code>[hlL]</code>) are also
 * accepted, but the "L" modifier is ignored as superfluous. Finally, all the
 * standard conversion-type characters (<code>[idfgGoxXeEcs]</code>) are
 * supported, except for "p" and "n".
 *<p>
 * In addition, PrintfFormat supports positional syntax for arguments, field
 * widths, and precisions. A format specifier that starts with a "%<i>n</i>$"
 * sequence will be applied to the <i>n</i>th argument. A field width or
 * precision of the form "*<i>m</i>$" will take its value from the <i>m</i>th
 * argument. The standard variable specifier for field width and precision (
 * <code>*</code>), which takes its value from the next argument, is also
 * supported. However, if a variable field width or precision is encountered in
 * a format spec that starts with a positional argument spec, the "*" will be
 * ignored and the default value will be used instead.
 * 
 * @author Allan Jacobs Release 1: Initial release. Release 2: Asterisk field
 *         widths and precisions %n$ and *m$ Bug fixes g format fix (2 digits in
 *         e form corrupt) rounding in f format implemented round up when digit
 *         not printed is 5 formatting of -0.0f round up/down when last digits
 *         are 50000...
 */
public class PrintfFormat
{
	 ...
	 * separated, or followed by ordinary strings.
	 * 
	 * @param fmtArg
	 *            Control string.
