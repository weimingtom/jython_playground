...
public class Glob
{
	...
	public Glob(String cwd, String pattern)
	{
		...
		// We pre-expand the pattern list into multiple patterns (see
		// GlobPattern for more info).
	...
	private static List splitPatternBraces(List dirs)
	{
		...
		// Our test condition is dynamic (dirs.size() each iteration), so this
		// is ok.
	...
	/**
	 * Get file objects for glob; made private to prevent it being used directly
	 * in the future
	 */
	...
	/*
	 * Glob breaks up a glob expression into multiple glob patterns. This is
	 * needed when dealing with glob patterns like '{/home,foo}/enebo/*.rb' or
	 * '/home/enebo/{foo,bar/}' So for every embedded '{}' pair we create a new
	 * glob pattern and then determine whether that pattern ends with a
	 * delimeter or is an absolute pattern. Pathological globs could lead to
	 * many many patterns (e.g. '{a{b,{d,e},{g,h}}}') where each nested set of
	 * curlies will double the amount of patterns.
	 * 
	 * Glob and GlobPattern do a lot of list copying and transforming. This
	...
	private class GlobPattern
	{
		...
		public GlobPattern(String cwd, String pattern)
		{
			...
			// Make a boolean for this special case (how about multiple
			// slashes?)
			...
			{
				// pattern is relative, but we need to consider cwd; add cwd but
				// remember it's relative so we chop it off later
