...
public class IOHandlerNio extends IOHandler
{
	...
	public void setBlocking(boolean block) throws IOException
	{
		...
		synchronized (((SelectableChannel) channel).blockingLock())
		{
			...
			{
				// ignore this; select() will set the correct mode when it is
				// finished
			}
		}
	}
	...
	{
		search_loop: for (int i = haystack.limit() - needle.length; i >= haystack.position(); i--)
		{
	...
	{
		search_loop: for (int i = haystack.position(); i + (needle.length - 1) < haystack.limit(); i++)
		{
			for (int j = 0; j < needle.length; j++)
			{
				if (haystack.get(i + j) != needle[j])
	...
	/*
	 * FIXME: what is the difference between sync() and flush()? does this
	 * difference make sense for nio?
	 */
	...
	{
		// FIXME: Satisfied for IOHandler, but this should throw some
