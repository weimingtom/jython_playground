...
public class DefinedVisitor extends AbstractVisitor
{
	...
	public Instruction visitCallNode(CallNode iVisited)
	{
		if (getDefinition(iVisited.getReceiverNode()) != null)
		{
			try
			{
				...
				if (!visibility.isPrivate() && (!visibility.isProtected() || state.getSelf().isKindOf(metaClass.getRealClass())))
				{
	...
	{
		if (state.getThreadContext().getRubyClass() == null && state.getSelf().getMetaClass().isClassVarDefined(iVisited.getName()))
		{
			definition = "class_variable";
		} else if (!state.getThreadContext().getRubyClass().isSingleton() && state.getThreadContext().getRubyClass().isClassVarDefined(iVisited.getName()))
		{
			definition = "class_variable";
		} else
		{
			RubyModule module = (RubyModule) state.getThreadContext().getRubyClass().getInstanceVariable("__attached__");
