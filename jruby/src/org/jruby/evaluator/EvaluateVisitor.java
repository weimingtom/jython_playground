...
public final class EvaluateVisitor implements NodeVisitor
{
	...
	// Collapsed; use of stack to aggregate args could be improved (see FIXME in
	// EvaluationState#aggregateResult) but it's not bad
	private static class ArgsCatNodeVisitor2 implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			RubyArray list = args instanceof RubyArray ? (RubyArray) args : state.runtime.newArray(args);
	...
	// Collapsed; use of stack to aggregate Array elements could be optimized a
	// bit
	...
	// CON20060104: This actually may not be a big deal, since the RubyRegexp
	// functions are lightweight and run inline
	...
	private static class BlockPassNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			if (block != null)
			{
				...
				// The current block is already associated with the proc. No
				// need to create new
			...
			while (firstWhenNode != null)
			{
				...
				if (whenNode.getExpressionNodes() instanceof ArrayNode)
				{
					for (Iterator iter = ((ArrayNode) whenNode.getExpressionNodes()).iterator(); iter.hasNext();)
					{
						...
						// Ruby grammar has nested whens in a case body because
						// of
						...
						if (tag instanceof WhenNode)
						{
							...
							for (int j = 0; j < expressions.getLength(); j++)
							{
								...
								if ((expression != null && condition.callMethod("===", expression).isTrue()) || (expression == null && condition.isTrue()))
								{
						...
						if ((expression != null && state.getResult().callMethod("===", expression).isTrue()) || (expression == null && state.getResult().isTrue()))
						{
	...
	private static class Colon2NodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			// rule in grammar (it is convenient to think of them as the same
			// thing
	...
	private static class DStrNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			for (ListIterator iterator = iVisited.reverseIterator(); iterator.hasPrevious();)
			{
				...
				// FIXME: skipping null node...but why would we find null nodes
				// in here?!
				if (iterNode == null)
					continue;
	...
	private static class DSymbolNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			for (ListIterator iterator = iVisited.getNode().reverseIterator(); iterator.hasPrevious();)
			{
				Node iterNode = (Node) iterator.previous();
	...
	private static class DefnNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			DefaultMethod newMethod = new DefaultMethod(containingClass, iVisited.getBodyNode(), (ArgsNode) iVisited.getArgsNode(), visibility, state.getThreadContext().peekCRef());
			...
			// 'class << state.self' and 'class << obj' uses defn as opposed to
			// defs
	...
	private static class DefsNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			{
				throw state.runtime.newTypeError("can't define singleton method \"" + iVisited.getName() + "\" for " + receiver.getType());
			}
			...
			DefaultMethod newMethod = new DefaultMethod(rubyClass, iVisited.getBodyNode(), (ArgsNode) iVisited.getArgsNode(), Visibility.PUBLIC, state.getThreadContext().peekCRef());
	...
	private static class FlipNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			if (iVisited.isExclusive())
			{
				...
				{
					// Benoit: I don't understand why the state.result is
					// inversed
	...
	private static class IfNodeImplVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			// Must set to nil; ifs or logical statements without then/else
			// return nil
			state.clearResult();
	...
	private static class IterNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			state.getThreadContext().preIterEval(Block.createBlock(iVisited.getVarNode(), new EvaluateCallable(iVisited.getBodyNode(), iVisited.getVarNode()), state.getSelf()));
			try
			{
				while (true)
				{
	...
	private static class NewlineNodeVisitor implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			// Newlines flush result (result from previous line is not available
			// in next line...perhaps makes sense)
			state.clearResult();
		}
	}
	...
	// Nothing to do
	// FIXME: This is called for "NilNode" visited...shouldn't NilNode visits
	// setResult(nil)?
	...
	private static class OptNNodeGets implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			// FIXME: exceptions could bubble out of this, but no exception
			// handling available in system yet..
	...
	private static class UntilConditionCheck implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			if (!condition.isTrue())
			{
				...
				if (iVisited.getBodyNode() != null)
				{
					// FIXME: Hack?...bodynode came up as null for lex method in
					// irb's ruby-lex.rb
					state.addRedoMarker(iVisited.getBodyNode());
					state.addNodeInstruction(iVisited.getBodyNode());
				}
	...
	private static class WhileConditionCheck implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			...
			if (condition.isTrue())
			{
				...
				state.addNodeInstruction(iVisited.getConditionNode());
				// FIXME: Hack? See UntilConditionCheck for explanation of why
