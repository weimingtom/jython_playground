...
public class EvaluationState
{
	...
	/**
	 * Pop and discard the top item on the current top stacks of nodes and
	 * visitors
	 */
	...
	/**
	 * Add a node and visitor to the top node and visitor stacks, using the
	 * default visitor for the given node. The default visitor is determined by
	 * calling node.accept() with the current state's evaluator.
	 * 
	 * @param ctx
	 */
	...
	/**
	 * Add the specified node and visitor to the top of the node and visitor
	 * stacks. The node will be passed to the visitor when it is evaluated.
	 * 
	 * @param node
	 * @param visitor
	 */
	...
	private static class Retrier implements Instruction
	{
		public void execute(EvaluationState state, InstructionContext ctx)
		{
			// dummy, only used to store the current "retriable" node and clear
			// exceptions after a rescue block
	...
	/**
	 * Call the topmost visitor in the current top visitor stack with the
	 * topmost node in the current top node stack.
	 */
	public void executeNext()
	{
		try
		{
			// FIXME: Poll from somewhere else in the code? This polls per-node,
			// perhaps per newline?
	...
	public IRubyObject begin(Node node)
	{
		...
		try
		{
			// TODO: once we're ready to have an external entity run this loop
			// (i.e. thread scheduler) move this out
			while (hasNext())
			{
		...
		{
			// TODO: perhaps a better place to catch this (although it will go
			// away)
	...
	public void begin2(Node node)
	{
		...
		if (node != null)
		{
			// for each call to internalEval, push down new stacks (to isolate
			// eval runs that still want to be logically separate
	...
	private void handleRaise(JumpException je)
	{
		...
		// TODO: Rubicon TestKernel dies without this line. A cursory glance
		// implies we
		// falsely set $! to nil and this sets it back to something valid. This
		// should
		// get fixed at the same time we address bug #1296484.
		runtime.getGlobalVariables().set("$!", raisedException);

		// FIXME: don't use the raise rethrower; work with the exception
		// rethrower like all other handlers do
	...
	// Had to make it work this way because eval states are sometimes created in
	// one thread for use in another...
	// For example, block creation for a new Thread; block, frame, and evalstate
	// for that Thread are created in the caller
