...
public class AssignmentVisitor extends AbstractVisitor
{
	...
	{
		if (state.runtime.getVerbose().isTrue() && state.getThreadContext().getRubyClass().isSingleton())
		{
			state.runtime.getWarnings().warn(iVisited.getPosition(), "Declaring singleton class variable.");
		}
		state.getThreadContext().getRubyClass().setClassVar(iVisited.getName(), value);
		return null;
	}
