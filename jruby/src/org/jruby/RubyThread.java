...
public class RubyThread extends RubyObject
{
	...
	private static RubyThread startThread(final IRubyObject recv, final IRubyObject[] args, boolean callInit)
	{
		...
		// make sure the thread has started before continuing, so it will appear
		// "runnable" to the rest of Ruby
	...
	public RubyThread join()
	{
		...
		try
		{
			...
			if (criticalThread != null)
			{
				// set the target thread's joinedBy, so it knows it can execute
	...
	{
		// FIXME: This should probably do some translation from Ruby priority
		// levels to Java priority levels (until we have green threads)
	...
	public IRubyObject run()
	{
		...
		// Abort any sleep()s
		// CON: Sleep now waits on the same stoplock, so it will have been woken
		// up by the notify above
	...
	public IRubyObject kill()
	{
		...
		synchronized (this)
		{
			...
			try
			{
				...
				{
					// we did not interrupt the thread, so wait for it to
					// complete
					// TODO: test that this is correct...should killer wait for
					// killee to die?
					threadImpl.join();
				}
			} catch (InterruptedException ie)
	...
	public void exceptionRaised(RaiseException exception)
	{
		...
		if (abortOnException(runtime))
		{
			...
			// TODO: Doesn't SystemExit have its own method to make this less
			// wordy..
