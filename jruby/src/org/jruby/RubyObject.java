...
public class RubyObject implements Cloneable, IRubyObject
{
	...
	/**
	 * This method is just a wrapper around the Ruby "==" method, provided so
	 * that RubyObjects can be used as keys in the Java HashMap object
	 * underlying RubyHash.
	 */
	...
	{
		// TODO: Can we assert MetaClass on metaClass here? This should simplify
		// some callers
		if (isNil())
		{
     ...
     * 
     */
	public IRubyObject callMethod(String name, IRubyObject[] args, CallType callType)
	{
     ...
     * 
     */
	public IRubyObject callMethod(RubyModule context, String name, IRubyObject[] args, CallType callType)
	{
		assert args != null;
		...
		if (method.isUndefined() || !(name.equals("method_missing") || method.isCallableFrom(getRuntime().getCurrentContext().getCurrentFrame().getSelf(), callType)))
		{
		...
		// System.out.println("method " + name + " found in " +
		// (implementer.isIncluded()?"included module ":"module or class ") +
		// implementer.getName());
	...
	public IRubyObject instance_variable_get(IRubyObject var)
	{
		...
		// Pickaxe v2 says no var should show NameError, but ruby only sends
		// back nil..
	...
	{
		return setInstanceVariable(name, value, "Insecure: can't modify instance variable", "");
	}
	...
	public IRubyObject eval(Node n)
	{
		...
		// need to continue evaluation with a new self, so save the old one
		// (should be a stack?)
	...
	public IRubyObject convertToType(String targetType, String convertMethod, boolean raise)
	{
		...
		if (!respondsTo(convertMethod))
		{
			if (raise)
			{
				throw getRuntime().newTypeError("cannot convert " + getMetaClass().getName() + " into " + targetType);
				// FIXME nil, true and false instead of NilClass, TrueClass,
				// FalseClass;
			}
	...
	public IRubyObject convertType(Class type, String targetType, String convertMethod)
	{
		...
		{
			throw getRuntime().newTypeError(getMetaClass().getName() + "#" + convertMethod + " should return " + targetType + ".");
		}
	...
	public void checkSafeString()
	{
		...
		{
			throw getRuntime().newTypeError("wrong argument type " + getMetaClass().getName() + " (expected String)");
		}
	}
	...
	public IRubyObject specificEval(RubyModule mod, IRubyObject[] args)
	{
		...
		} else if (args.length > 3)
		{
			...
			throw getRuntime().newArgumentError("wrong # of arguments: " + lastFuncName + "(src) or " + lastFuncName + "{..}");
		}
		/*
		 * if (ruby.getSecurityLevel() >= 4) { Check_Type(argv[0], T_STRING); }
		 * else { Check_SafeStr(argv[0]); }
		 */
	...
	{
		/*
		 * if (ruby_safe_level >= 4) { Check_Type(src, T_STRING); } else {
		 * Check_SafeStr(src); }
		 */
		return under.executeUnder(new Callback() {
			public IRubyObject execute(IRubyObject self, IRubyObject[] args)
			{
				...
				return args[0].eval(source, self.getRuntime().getNil(), ((RubyString) filename).toString(), RubyNumeric.fix2int(lineNumber));
			}
	...
	public IRubyObject eval(IRubyObject src, IRubyObject scope, String file, int line)
	{
		...
		try
		{
			...
			{
				threadContext.pushRubyClass(this instanceof RubyModule ? (RubyModule) this : this.getType());
	 ...
	 * 
	 * Returns true if this object responds to the given method. Private methods
	 ...
	 * 
	 * <b>Warning:</b> In JRuby there is no guarantee that two objects have
	 * different ids.
	 * 
	...
	// Hack: allow RubyModule and RubyClass to override the allocation and
	// return the the correct Java instance
	// Cloning a class object doesn't work otherwise and I don't really
	// understand why --sma
	...
	{
		IRubyObject port = args.length == 0 ? getRuntime().getGlobalVariables().get("$>") : args[0];
	 ...
	 * 
	 */
	// TODO: This is almost RubyModule#instance_methods on the metaClass.
	// Perhaps refactor.
	...
	public IRubyObject method_missing(IRubyObject[] args)
	{
		...
		String msg = new PrintfFormat(format).sprintf(new Object[] { name, description, noClass ? "" : ":", noClass ? "" : getType().getName() });
	...
	public void marshalTo(MarshalStream output) throws java.io.IOException
	{
		...
		for (Iterator iter = instanceVariableNames(); iter.hasNext();)
		{
			...
			// Between getting name and retrieving value the instance variable
			// could have been
			// removed
			if (value != null)
			{
