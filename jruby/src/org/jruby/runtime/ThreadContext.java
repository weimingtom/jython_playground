...
public class ThreadContext
{
	...
	// TODO: This and the following version maybe can be combined after studying
	// usage patterns
	...
	private void pushFrame(IRubyObject self, IRubyObject[] args, String lastFunc, RubyModule lastClass)
	{
	...
	public IRubyObject callSuper(IRubyObject[] args)
	{
		...
		try
		{
			...
			if (superClass == null)
			{
				// TODO cnutter: I believe modules, the only ones here to have
				// no superclasses, should have Module as their superclass
				superClass = runtime.getClass("Module");
			}
			return frame.getSelf().callMethod(superClass, frame.getLastFunc(), args, CallType.SUPER);
		} finally
		{
			iterStack.pop();
		}
	...
	public IRubyObject yield(IRubyObject value, IRubyObject self, RubyModule klass, boolean yieldProc, boolean aValue)
	{
		...
		// block is executed under its own self, so save the old one (use a
		// stack?)
		...
		try
		{
			...
			while (true)
			{
				try
				{
					// FIXME: is it appropriate to use the current frame's (the
					// block's frame's) lastClass?
	...
	private IRubyObject[] getBlockArgs(IRubyObject value, IRubyObject self, boolean yieldProc, boolean aValue, Block currentBlock)
	{
		...
		if (blockVar != null)
		{
			if (blockVar instanceof ZeroArgNode)
			{
				...
				{
					throw runtime.newArgumentError("wrong # of arguments(" + ((RubyArray) value).getLength() + "for 0)");
				}
			...
			} else
			{
				if (aValue)
				{
					...
					} else
					{
						...
						// throw
						// runtime.newArgumentError("wrong # of arguments(" +
						// length + "for 1)");
					}
				} else if (value == null)
				{
					// XXXEnebo - Should be warning not error.
					// throw
					// runtime.newArgumentError("wrong # of arguments(0 for 1)");
				}
	...
	public IRubyObject getConstant(String name)
	{
		...
		// System.out.println("CREF is " +
		// state.threadContext.getCRef().getValue());
	...
	// XXX: This is screwy...apparently Ruby runs internally-implemented methods
	// in their own frames but in the *caller's* scope
	...
	public void preRunThread(Frame currentFrame, Block currentBlock)
	{
		...
		// create a new eval state for the the block frame (since it has been
		// adopted by the created thread)
		// XXX: this is kind of a hack, since eval state holds ThreadContext,
		// and when it's created it's in the other thread :(
		// we'll want to revisit these issues of block ownership since the block
		// is created in one thread and used in another
		// currentBlock.getFrame().setEvalState(new EvaluationState(runtime,
		// currentBlock.getFrame().getSelf()));
		pushdownBlocks(currentBlock);
	}

	public void preKernelEval()
	{
		// we pop here and push in the post so the eval runs under the previous
		// frame
