 ...
 * determining a set of valid method visibilities. The first concept can only be
 * a single value: PUBLIC, PRIVATE, PROTECTED, and MODULE_FUNCTION (see
 * RubyModule#module_function). It is used to adorn a method with a _SINGLE_
 * visibility. Some functions (see RubyModule#instance_methods) want to be able
 * to see methods of multiple visibilities. The second concept allows making a
 * vibility which is basically a mask (see Visibility.PUBLIC_PROTECTED). The
 * method 'is' can then be used to see if a method visibility is in the mask.
 * 
 * @author jpetersen
 */
public final class Visibility implements Serializable
{
	...
	public static final Visibility PUBLIC_PROTECTED = new Visibility((short) (PUBLIC_VALUE | PROTECTED_VALUE));
