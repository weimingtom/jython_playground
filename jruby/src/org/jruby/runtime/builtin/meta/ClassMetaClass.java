...
public class ClassMetaClass extends ObjectMetaClass
{
	 ...
	 * 
	 * protected IRubyObject allocateObject() { RubyClass instance = (RubyClass)
	 * newClass(IRubyObject.NULL_ARRAY);
	 * 
	 * instance.setMetaClass(this);
	 * 
	 * return instance; }
	 * 
	 * public IRubyObject newInstance(IRubyObject[] args) { return
	 * newClass(IRubyObject.NULL_ARRAY); }
	 * 
	 * public IRubyObject newClass(IRubyObject[] args) { Ruby runtime =
	 * getRuntime();
	 * 
	 * RubyClass superClass; if (args.length > 0) { if (args[0] instanceof
	 * RubyClass) { superClass = (RubyClass) args[0]; } else { throw
	 * runtime.newTypeError( "wrong argument type " +
	 * args[0].getType().getName() + " (expected Class)"); } } else { superClass
	 * = runtime.getClasses().getObjectClass(); }
	 * 
	 * RubyClass newClass = superClass.subclass();
	 * 
	 * newClass.makeMetaClass(superClass.getMetaClass(),
	 * runtime.getCurrentContext().getLastRubyClass());
	 * 
	 * // call "initialize" method newClass.callInit(args);
	 * 
	 * // call "inherited" method of the superclass
	 * newClass.inheritedBy(superClass);
	 * 
	 * if (runtime.getCurrentContext().isBlockGiven()) { runtime.yield(null,
	 * newClass, newClass, false); }
	 * 
	 * return newClass; }
	 * 
	 * public IRubyObject inherited(IRubyObject ignore) { return
	 * getRuntime().getNil(); }
	 */
}
