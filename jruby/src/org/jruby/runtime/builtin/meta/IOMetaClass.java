...
public class IOMetaClass extends ObjectMetaClass
{
	...
	public static IRubyObject select_static(IRuby runtime, IRubyObject[] args)
	{
		try
		{
			...
			{
				atLeastOneDescriptor = true;
				// Java's select doesn't do anything about this, so we leave it
				// be.
			}
			...
			for (Iterator i = selector.selectedKeys().iterator(); i.hasNext();)
			{
				...
				if ((key.interestOps() & key.readyOps() & (SelectionKey.OP_READ | SelectionKey.OP_ACCEPT | SelectionKey.OP_CONNECT)) != 0)
				{
