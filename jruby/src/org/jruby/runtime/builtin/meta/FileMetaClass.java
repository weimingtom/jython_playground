...
public class FileMetaClass extends IOMetaClass
{
	...
	protected class FileMeta extends Meta
	{
		protected void initializeClass()
		{
			...
			// TODO Singleton methods: atime, blockdev?, chardev?, chown, ctime,
			// directory?
			// TODO Singleton methods: executable?, executable_real?, extname,
			// TODO Singleton methods: ftype, grpowned?, lchmod, lchown, link,
			// mtime, owned?
			// TODO Singleton methods: pipe?, readlink, setgid?, setuid?,
			// socket?,
			// TODO Singleton methods: stat, sticky?, symlink, symlink?, umask,
			// utime
			...
			// TODO: Define instance methods: atime, chmod, chown, ctime,
			// lchmod, lchown, lstat, mtime
	...
	/**
	 * Returns true if path matches against pattern The pattern is not a regular
	 * expression; instead it follows rules similar to shell filename globbing.
	 * It may contain the following metacharacters: *: Glob - match any sequence
	 * chars (re: .*). If like begins with '.' then it doesn't. ?: Matches a
	 * single char (re: .). [set]: Matches a single char in a set (re: [...]).
	 * 
	 */
	...
	/*
	 * Fixme: This does not have exact same semantics as RubyArray.join, but
	 * they probably could be consolidated (perhaps as join(args[], sep,
	 * doChomp)).
	 */
	public RubyString join(IRubyObject[] args)
	{
		...
		for (int i = 0; i < args.length; i++)
		{
			...
			{
				// Fixme: Need infinite recursion check to put [...] and not go
				// into a loop
	...
	public IRubyObject open(IRubyObject[] args, boolean tryToYield)
	{
		...
		IOModes modes = args.length >= 2 ? getModes(args[1]) : new IOModes(runtime, IOModes.RDONLY);
	...
	public RubyArray split(IRubyObject arg)
	{
		...
		return filename.getRuntime().newArray(dirname(filename), basename(new IRubyObject[] { filename }));
	}

	public IRubyObject symlink_p(IRubyObject arg1)
	{
		// FIXME if possible, make this return something real (Java's lack of
		// support for symlinks notwithstanding)
	...
	/**
	 * This method does NOT set atime, only mtime, since Java doesn't support
	 * anything else.
	 */
