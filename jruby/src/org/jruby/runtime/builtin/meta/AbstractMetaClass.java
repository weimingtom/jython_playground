...
public abstract class AbstractMetaClass extends RubyClass
{
	protected abstract class Meta
	{
		...
		/**
		 * Base implementation uses the data-driven approach not used currently
		 * but possibly revisited in the future.
		 */
		...
		public void defineConstants(Object[][] constants, boolean singleton)
		{
			for (int i = 0; i < constants.length; i++)
			{
				if (singleton)
				{
					getSingletonClass().defineConstant((String) constants[i][0], (IRubyObject) constants[i][1]);
				} else
				{
					defineConstant((String) constants[i][0], (IRubyObject) constants[i][1]);
				}
			}
		}
		...
		public void setConstants(Object[][] constants, boolean singleton)
		{
			for (int i = 0; i < constants.length; i++)
			{
				if (singleton)
				{
					getSingletonClass().setConstant((String) constants[i][0], (IRubyObject) constants[i][1]);
				} else
				{
					setConstant((String) constants[i][0], (IRubyObject) constants[i][1]);
				}
			}
		}
	...
	protected AbstractMetaClass(String name, Class builtinClass, RubyClass superClass, SinglyLinkedList parentCRef, boolean init)
	{
		...
		// assert RubyObject.class.isAssignableFrom(builtinClass) ||
		// RubyObject.class == builtinClass:
		// "builtinClass have to be a subclass of RubyObject.";
		assert superClass != null;

		this.builtinClass = builtinClass;

		makeMetaClass(superClass.getMetaClass(), superClass.getRuntime().getCurrentContext().peekCRef());
