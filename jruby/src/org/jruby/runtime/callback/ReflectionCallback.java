...
/**
 * A wrapper for <code>java.lang.reflect.Method</code> objects which implement
 * Ruby methods.
 */
public class ReflectionCallback implements Callback
{
	...
	/**
	 * Returns an object array that collects all rest arguments in its own
	 * object array which is then put into the last slot of the first object
	 * array. That is, assuming that this callback expects one required argument
	 * and any number of rest arguments, an input of <code>[1, 2, 3]</code> is
	 * transformed into <code>[1, [2, 3]]</code>.
	 */
	...
	/**
	 * Invokes the Ruby method. Actually, this methods delegates to an internal
	 * version that may throw the usual Java reflection exceptions. Ruby
	 * exceptions are rethrown, other exceptions throw an AssertError and abort
	 * the execution of the Ruby program. They should never happen.
	 */
	/**
	 * Calls a wrapped Ruby method for the specified receiver with the specified
	 * arguments.
	 */
	public IRubyObject execute(IRubyObject recv, IRubyObject[] oargs)
	{
		...
		} catch (final IllegalArgumentException e)
		{
			 ...
			 * message.append(" recv=").append(recv.toString());
			 * message.append(" type=").append(type.getName());
			 * message.append(" methodArgs=["); for (int i = 0; i <
			 * methodArgs.length; i++) { message.append(methodArgs[i]);
