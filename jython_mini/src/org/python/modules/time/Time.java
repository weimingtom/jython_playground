...
public class Time implements ClassDictInit
{
	...
	public static void classDictInit(PyObject dict)
	{
		...
		tzname = new PyTuple(new PyObject[] { new PyString(getDisplayName(tz, false, 0)), new PyString(getDisplayName(tz, true, 0)) });
	...
	protected static PyTimeTuple _timefields(double secs, TimeZone tz)
	{
		...
		return new PyTimeTuple(new PyObject[] { new PyInteger(cal.get(Calendar.YEAR)), new PyInteger(cal.get(Calendar.MONTH) + 1), new PyInteger(cal.get(Calendar.DAY_OF_MONTH)), new PyInteger(cal.get(Calendar.HOUR) + 12 * cal.get(Calendar.AM_PM)), new PyInteger(cal.get(Calendar.MINUTE)), new PyInteger(cal.get(Calendar.SECOND)), new PyInteger(dow), new PyInteger(cal.get(Calendar.DAY_OF_YEAR)), new PyInteger(isdst ? 1 : 0) });
	}
	...
	private static String[] enshortdays = new String[] { 
		"Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun" 
	};
	private static String[] enshortmonths = new String[] { 
		...
		while (lastc < format.length())
		{
			...
			switch (format.charAt(i))
			{
				...
				case 'x':
					...
					// %X == HH:mm:SS
					//
					s = s + _twodigit(item(tup, 1) + 1) + "/" + _twodigit(item(tup, 2)) + "/" + _truncyear(item(tup, 0));
					break;
