...
{
	/*
	 * Generated from Python-2.2.3 like 'python headerToJava.py <
	...
	final boolean SRE_CHARSET(char[] set, int setidx, char ch)
	{
		...
		for (;;)
		{
			...
			{
				case SRE_OP_LITERAL:
					// TRACE(setidx, ch, "CHARSET LITERAL " + (int)
					// set[setidx]);
					/* <LITERAL> <code> */
					if (ch == set[setidx])
					{
				...
				case SRE_OP_RANGE:
					/* <RANGE> <lower> <upper> */
					// TRACE(setidx, ch, "CHARSET RANGE " + (int) set[setidx] +
					// " " + (int) set[setidx+1]);
				...
				case SRE_OP_CATEGORY:
					/* <CATEGORY> <code> */
					// TRACE(setidx, ch, "CHARSET CHARSET " + (int)
					// set[setidx]);
	...
	private int SRE_COUNT(char[] pattern, int pidx, int maxcount, int level)
	{
		...
		switch (pattern[pidx])
		{
			...
			case SRE_OP_ANY_ALL:
				/*
				 * repeated dot wildcare. skip to the end of the target string,
	...
	final int SRE_MATCH(char[] pattern, int pidx, int level)
	{
		...
		for (;;)
		{
			switch (pattern[pidx++])
			{
				...
				case SRE_OP_LITERAL_IGNORE:
					// TRACE(pidx, ptr, "LITERAL_IGNORE " + (int)
					// pattern[pidx]);
				...
				case SRE_OP_NOT_LITERAL_IGNORE:
					// TRACE(pidx, ptr, "NOT_LITERAL_IGNORE " + (int)
					// pattern[pidx]);
					if (ptr >= end || lower(str[ptr]) == lower(pattern[pidx]))
					{
						return 0;
				...
				case SRE_OP_REPEAT_ONE:
					 ...
					 * matches, and ptr points to the tail of the target string.
					 * check if the rest of the pattern matches, and backtrack
					 * if not.
					 */
					if (count < mincount)
					{
					...
					{
						/*
						 * tail starts with a literal. skip positions where the
						 * rest of the pattern cannot possibly match
						 */
				...
				case SRE_OP_REPEAT:
					/*
					 * create repeat context. all the hard work is done by the
					 * UNTIL operator (MAX_UNTIL, MIN_UNTIL)
					 */
					/* <REPEAT> <skip> <1=min> <2=max> item <UNTIL> tail */
					// TRACE(pidx, ptr, "REPEAT " + (int)pattern[pidx+1] + " " +
					// (int)pattern[pidx+2]);
				...
				case SRE_OP_MAX_UNTIL:
					...
					/*
					 * FIXME: we probably need to deal with zero-width matches
					 * in here...
					 */
					...
					{
						/*
						 * we may have enough matches, but if we can match
						 * another item, do so
						 */
						rp.count = count;
						lastmark = this.lastmark;
