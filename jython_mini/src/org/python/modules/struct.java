 ...
 * <tt>'10s'</tt> means a single 10-byte string, while <tt>'10c'</tt> means 10 characters. For
 * packing, the string is truncated or padded with null bytes as appropriate to
 * make it fit. For unpacking, the resulting string always has exactly the
 * specified number of bytes. As a special case, <tt>'0s'</tt> means a single, empty
 ...
 * count of zero, e.g. the format <tt>'llh0l'</tt> specifies two pad bytes at the end,
 * assuming longs are aligned on 4-byte boundaries. This only works when native
 * size and alignment are in effect; standard size and alignment does not
