...
public class imp
{
	...
	public static PyObject load_source(String modname, String filename)
	{
		...
		// XXX: bufsize is ignored in PyFile now, but look 3rd arg if this ever
		// changes.
		...
		mod = org.python.core.imp.createFromSource(modname.intern(), (InputStream) o, filename.toString());
	...
	public static PyObject find_module(String name, PyObject path)
	{
		...
		for (PyObject p = null; (p = iter.__iternext__()) != null;)
		{
			...
			return new PyTuple(new PyObject[] { mi.file, new PyString(mi.filename), new PyTuple(new PyObject[] { new PyString(mi.suffix), new PyString(mi.mode), Py.newInteger(mi.type) }), });
		}
	...
	public static PyObject load_module(String name, PyObject file, PyObject filename, PyTuple data)
	{
		...
		while (mod == Py.None)
		{
			...
			{
				case PY_SOURCE:
					mod = org.python.core.imp.createFromSource(name.intern(), (InputStream) o, filename.toString());
					break;
					
				case PY_COMPILED:
					mod = org.python.core.imp.loadFromCompiled(name.intern(), (InputStream) o, filename.toString());
					break;
	...
	{
		return new PyList(new PyObject[] { new PyTuple(new PyObject[] { new PyString(".py"), new PyString("r"), Py.newInteger(PY_SOURCE), }), new PyTuple(new PyObject[] { new PyString(".class"), new PyString("rb"), Py.newInteger(PY_COMPILED), }), });
	}
