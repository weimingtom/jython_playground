...
public class PyString extends PyBaseString implements ClassDictInit
{
						...
						if (endBrace != end && str.charAt(endBrace) == '}')
						{
							...
							} else
							{
								...
								} else
								{
									...
									/*
									 * high surrogate = top 10 bits added to
									 * D800
									 */
									v.append((char) (0xD800 + (value >> 10)));

									/*
									 * low surrogate = bottom 10 bits added to
									 * DC00
									 */
	...
	final PyList str_splitlines(boolean keepends)
	{
		...
		for (int i = 0; i < n;)
		{
			...
			while (i < n && chars[i] != '\n' && chars[i] != '\r' && Character.getType(chars[i]) != Character.LINE_SEPARATOR)
				i++;
	...
	public PyString format(PyObject args)
	{
		...
		while (index < format.length())
		{
			...
			switch (c)
			{
				...
				default:
					throw Py.ValueError("unsupported format character '" + codecs.encode(Py.newString(c), null, "replace") + "' (0x" + Integer.toHexString(c) + ") at index " + (index - 1));
			}
			int length = string.length();
			int skip = 0;
