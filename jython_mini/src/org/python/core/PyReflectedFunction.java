...
public class PyReflectedFunction extends PyObject
{
	...
	public PyObject __call__(PyObject self, PyObject[] args, String[] keywords)
	{
		...
		if (self == null && cself != null && cself instanceof PyProxy && !__name__.startsWith("super__"))
		{
			...
			if (argslist[0].declaringClass != iself.instclass.proxyClass)
			{
				...
				/*
				 * xxx this way it is slow! Method super_method = null; try {
				 * super_method =
				 * cself.getClass().getMethod(mname,m.getParameterTypes()); }
				 * catch(NoSuchMethodException e) { // ??? more stuff to ignore?
				 * } if (super_method != null) { m = super_method; }
				 */
				/*
				 * xxx original PyJavaClass jc =
				 * PyJavaClass.lookup(iself.__class__.proxyClass); PyObject
				 * super__ = jc.__findattr__(mname.intern()); if (super__ !=
				 * null) { return super__.__call__(self, args, keywords); }
				 */
			}
		}
		try
	...
	protected void throwError(int errArg, int nArgs, boolean self, boolean keywords)
	{
		...
		/*
		 * if (errArg == -1) { throwBadArgError(-1);
		 * throwError("bad self argument"); // Bad declared class }
		 */
