...
public class PyDictionary extends PyObject
{
	 ...
	 * 
	 * @param t
	 *            the hashtable used. The supplied hashtable is used as is and
	 *            must only contain PyObject key:value pairs.
	 */
				...
				for (; (pair = pairs.__iternext__()) != null; cnt++)
				{
					...
					} catch (PyException e)
					{
						if (Py.matchException(e, Py.TypeError))
							throw Py.TypeError("cannot convert dictionary update " + "sequence element #" + cnt + " to a sequence");
						throw e;
					}
					int n;
					if ((n = pair.__len__()) != 2)
					{
						throw Py.ValueError("dictionary update sequence element #" + cnt + " has length " + n + "; 2 is required");
					}
	...
	/*
	 * commenting this out -- PyObject.safeRepr() does the same thing, and this
	 * one messes up subclasses of dict. XXX: delete all of this if this turns
	 * out okay.
	 * 
	 * public String safeRepr() throws PyIgnoreMethodTag { return
	 * "'dict' object"; }
	 */
	...
	/**
	 * Return this[key] if the key exists in the mapping, default_object is
	 * returned otherwise.
	 * 
	 * @param key
