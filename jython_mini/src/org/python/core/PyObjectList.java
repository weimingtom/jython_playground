...
/**
 * <CODE>java.util.List</CODE> implementation using an underlying PyObject array
 * for higher performance. Jython should use the following methods where
 * possible, instead of their <CODE>List</CODE> counterparts:
 * <UL>
...
extends AbstractList implements /* RandomAccess, */Cloneable, Serializable
{
	...
	/*
	 * Design note: This class let's PySequenceList implement java.util.List by
	 * delegating to an instance of this. The major distinction is that the
	 * backing array is PyObject[], not Object[] (as you'd get by delegating to
	 * ArrayList). There are 2 major benefits: 1) A lot of casting can be
	 * avoided internally (although use of PySequenceList descendants as java
	 * collections does involve some casting); 2) PySequenceList descendants can
	 * still do bulk array operations, allowing better performance and reuse of
	 * much of the pre-collections bulk operation implementation.
	 */
	...
	/**
	 * Get the backing array. The array should generally not be modified. To get
	 * a copy of the array, see {@link #toArray()} which returns a copy.
	 * 
