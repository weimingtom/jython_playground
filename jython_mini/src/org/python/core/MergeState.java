...
/**
 * The MergeState class is a java implementation of the sort created Tim Peters
 * and added to CPython2.3.
 * 
...
{
	/**
	 * The maximum number of entries in a MergeState's pending-runs stack. This
	 * is enough to sort arrays of size up to about 32 * phi **
	 * MAX_MERGE_PENDING where phi ~= 1.618. 85 is ridiculously large enough,
	 * good for an array with 2**64 elements.
	 */
	static final int MAX_MERGE_PENDING = 85;

	/**
	 * If a run wins MIN_GALLOP times in a row, we switch to galloping mode, and
	 * stay there until both runs win less often than MIN_GALLOP consecutive
	 * times. See listsort.txt for more info.
	 */
	 ...
	 * left of the leftmost equal element. [gallop_right() does the same except
	 * returns the position to the right of the rightmost equal element (if
	 * any).]
	 * 
	...
	private int gallop_left(PyObject key, PyObject[] localData, int localA, int localN, int hint)
	{
		...
		{
			/*
			 * a[hint] < key -- gallop right, until a[hint + lastofs] < key <=
			 * a[hint + ofs]
			 */
		...
		/*
		 * Now a[lastofs] < key <= a[ofs], so key belongs somewhere to the right
		 * of lastofs but no farther right than ofs. Do a binary search, with
	 ...
	 * 
	 * The return value is the int k in 0..n such that a[k-1] <= key < a[k] or
	 * -1 if error.
	 * 
	...
	private int gallop_right(PyObject key, PyObject[] aData, int localA, int localN, int hint)
	{
		...
		{
			/*
			 * key < a[hint] -- gallop left, until a[hint - ofs] <= key < a[hint
			 * - lastofs]
			 */
			int maxofs = hint + 1; /* data[a] is lowest */
			while (ofs < maxofs)
	 ...
	 * 
	 * If n < 64, return n (it's too small to bother with fancy stuff). Else if
	 * n is an exact power of 2, return 32. Else return an int k, 32 <= k <= 64,
	...
	private boolean iflt(PyObject x, PyObject y)
	{
		...
		{
			/*
			 * NOTE: we rely on the fact here that the sorting algorithm only
			 * ever checks whether k<0, i.e., whether x<y. So we invoke the rich
			 * comparison function with _lt ('<'), and return -1 when it returns
			 * true and 0 when it returns false.
			 */
	 ...
	 * insertion. This sort is stable. On entry, must have lo <= start <= hi,
	 * and that [lo, start) is already sorted (pass start == lo if you don't
	 * know!). If islt() complains return -1, else 0. Even in case of error, the
	 * output slice will be some permutation of the input (nothing is lost or
	 * duplicated).
	 */
	...
	/*
	 * //debugging methods. private void dump_data(String txt, int lo, int n) {
	 * System.out.print(txt + ":"); for (int i = 0; i < n; i++)
	 * System.out.print(data[lo + i] + " "); System.out.println(); } private
	 * void debug(String str) { //System.out.println(str); }
	 */
}
