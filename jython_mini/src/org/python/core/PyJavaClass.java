...
public class PyJavaClass extends PyClass
{
	...
	{
		/*
		 * xxx disable opt, will need similar opt for types if
		 * (!PyObject.class.isAssignableFrom(c)) return; try { Field field =
		 * c.getField("__class__"); if (Modifier.isStatic(field.getModifiers())
		 * && field.getType().isAssignableFrom(PyJavaClass.class) &&
		 * field.getDeclaringClass() == c) { field.set(null, this); } } catch
		 * (NoSuchFieldException exc) {} catch (IllegalAccessException exc1) {}
		 */
	}
	...
	/**
	 * Return the list of all accessible interfaces for a class. This will only
	 * the public interfaces. Since we can't set accessibility on interfaces,
	 * the Options.respectJavaAccessibility is not honored.
	 */
	...
	/**
	 * Return the list of all accessible fields for a class. This will only be
	 * the public fields unless Options.respectJavaAccessibility is false, in
	 * which case all fields are returned.
	 */
	...
	/*
	 * Produce a good Python name for a Java method. If the Java method ends in
	 * '$', strip it (this handles reserved Java keywords) Don't make any
	 * changes to keywords since this is now handled by parser
	 */
	...
	private void addMethod(Method meth)
	{
		...
		{
			if (name == "layout" || name == "insets" || name == "size" || name == "minimumSize" || name == "preferredSize" || name == "maximumSize" || name == "bounds" || name == "enable")
			{
				return;
			}
		}
		...
		if (o != null && o instanceof PyReflectedFunction)
		{
			 ...
			 * signature. If this alters the signature of the function in some
			 * significant way, then return a duplicate and stick it in the
			 * __dict__
			 */
			if (o1 != o)
			{
	...
	/**
	 * Return the list of all accessible methods for a class. This will only the
	 * public methods unless Options.respectJavaAccessibility is false, in which
	 * case all methods are returned.
	 */
		...
		if (o != null)
		{
			...
			if (o instanceof PyBeanProperty)
			{
				...
				if (prop.myType == oldProp.myType)
				{
					...
					if ((prop.getMethod == null || oldProp.getMethod != null) && (prop.setMethod == null || oldProp.setMethod != null))
					{
						set = false;
					}
	...
	/*
	 * A reimplementation of java.beans.Introspector.decapitalize. This is
	 * needed due to bugs in Netscape Navigator
	 */
	...
	private void setBeanInfoCustom(Class c, Method[] meths)
	{
		...
		/*
		 * } catch (Throwable t) { System.err.println("Custom Bean error: "+t);
		 * t.printStackTrace(); }
		 */
	}

	/**
	 * Return the list of all accessible constructors for a class. This will
	 * only the public constructors unless Options.respectJavaAccessibility is
	 * false, in which case all constructors are returned. Note that
	 * constructors are not inherited like methods or fields.
	 */
	...
	private static String unmangleKeyword(String name)
	{
		if (keywords == null)
		{
			...
			String[] words = new String[] { "or", "and", "not", "is", "in", "lambda", "if", "else", "elif", "while", "for", "try", "except", "def", "class", "finally", "print", "pass", "break", "continue", "return", "import", "from", "del", "raise", "global", "exec", "assert" };
