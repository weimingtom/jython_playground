...
public final class Py
{
	...
	/*
	 * public static PyException SystemExit(String message) { return new
	 * PyException(Py.SystemExit, message); }
	 */
	...
	public static void memory_error(OutOfMemoryError t)
	{
		...
		// this logic would allow to re-enable the old behavior when it makes
		// sense,
	...
	public static void warning(PyObject category, String message, String filename, int lineno, String module, PyObject registry)
	{
		...
		{
			func.__call__(new PyObject[] { Py.newString(message), category, Py.newString(filename), Py.newInteger(lineno), (module == null) ? Py.None : Py.newString(module), registry }, Py.NoKeywords);
		}
	}
	...
	/*
	 * These are not used anymore. Uncomment them if there is a future clamor to
	 * make this functionality more easily usable public static void
	 * pushFrame(PyFrame f) { ThreadState ts = getThreadState(); f.f_back =
	 * ts.frame; if (f.f_builtins == null) f.f_builtins = f.f_back.f_builtins;
	 * ts.frame = f; }
	 * 
	 * public static PyFrame popFrame() { ThreadState ts = getThreadState();
	 * PyFrame f = ts.frame.f_back; ts.frame = f; return f; }
	 */
...
class JavaCode extends PyCode
{
	...
	public PyObject call(PyObject self, PyObject args[], String keywords[], PyObject globals, PyObject[] defaults, PyObject closure)
	{
