...
public class PyClass extends PyObject
{
	...
	void init(String name, PyTuple bases, PyObject dict)
	{
			...
			for (int i = 0; i < bases.size(); i++)
			{
				...
				if (proxy != null)
				{
					...
					} else
					{
						if (baseClass != null)
						{
							throw Py.TypeError("no multiple inheritance " + "for Java classes: " + proxy.getName() + " and " + baseClass.getName());
						}
		...
		if (proxyClass != null)
		{
			...
			PyObject superDict = PyJavaClass.lookup(proxyClass).__findattr__("__dict__"); // xxx
																							// getDict
																							// perhaps?
