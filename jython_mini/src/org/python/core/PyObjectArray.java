...
public class PyObjectArray extends AbstractArray
{
	...
	public PyObject get(int index)
	{
		...
		String message = (size == 0) ? "No data was added, unable to get entry at " + index : "Index must be between " + 0 + " and " + (size - 1) + ", but was " + index;
	 ...
	 * {@link #toArray()} which returns a copy. Note that <CODE>getSize()</CODE>
	 * should be used to determine the number of elements in the array, not the
	 * array's length (which may reflect excess capacity).
	...
	public PyObject set(int index, PyObject value)
	{
		...
		throw new ArrayIndexOutOfBoundsException("Index must be between " + 0 + " and " + (size - 1) + ", but was " + index);
	}
	 ...
	 * this growable array. The array's length matches the value returned by
	 * <CODE>getSize()</CODE>
	 * 
