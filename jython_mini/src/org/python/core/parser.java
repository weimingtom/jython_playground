...
public class parser
{
	...
	{
		return parse(new ByteArrayInputStream(PyString.to_bytes(string)), kind, "<string>", null);
	}
	...
	public static modType partialParse(String string, String kind, String filename, CompilerFlags cflags, boolean stdprompt)
	{
		...
		BufferedReader bufreader = prepBufreader(new ByteArrayInputStream(PyString.to_bytes(string)), cflags);
		...
		} catch (Throwable t)
		{
			 ...
			 * we check whether all sentence tokens have been consumed or the
			 * remaining ones fullfill lookahead expectations. See:
	...
	private static BufferedReader prepBufreader(InputStream istream, CompilerFlags cflags)
	{
		...
		} else
		{
			try
			{
				// Use ISO-8859-1 to get bytes off the input stream since it
				// leaves their values alone.
...
/**
 * A workaround for a bug in MRJ2.2's FileReader, where the value returned from
 * read(b, o, l) sometimes are wrong.
 */
