...
public class PyType extends PyObject implements Serializable
{
	...
	{
		return getLayout().equals(other.getLayout()) && needs_userdict == other.needs_userdict && needs_finalizer == other.needs_finalizer;
	}
	...
	public static PyObject newType(PyNewWrapper new_, PyType metatype, String name, PyTuple bases, PyObject dict)
	{
		...
		if (tmp != null && tmp instanceof PyFunction)
		{ // xxx java functions?
	...
	private static boolean necessitatesUserdict(PyObject[] bases_list)
	{
		...
		{
			PyObject cur = bases_list[i];
			if ((cur instanceof PyType && ((PyType) cur).needs_userdict && ((PyType) cur).numSlots > 0) || cur instanceof PyClass)
			{
				return true;
			}
		}
	...
	private static void fillFromClass(PyType newtype, String name, Class c, Class base, boolean newstyle, Method setup, String[] exposed_methods)
	{
	...
	private static void confirmIdentifier(PyObject o)
	{
		...
		if (identifier == null || identifier.length() < 1 || (!Character.isLetter(identifier.charAt(0)) && identifier.charAt(0) != '_'))
		{
			throw Py.TypeError(msg);
		}
