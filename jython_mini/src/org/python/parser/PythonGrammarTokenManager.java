...
public class PythonGrammarTokenManager implements PythonGrammarConstants
{
	...
	// control whether empty new lines on EOF force sentence closing NEWLINE
	// even if indent
	// is expected,i.e. classic behavior at jython prompt (different from codeop
	// behavior)
	...
	{
		/*
		 * if not partial: EOF is expanded to token sequences comprising if
		 * single_input: [NEWLINE] necessary DEDENT NEWLINE (afterward EOF)
		 * otherwise : [NEWLINE] necessary DEDENT EOF if partial: EOF expansion
		 * happens only if EOF preceded by empty line (etc), i.e. lexer is in
		 * MAYBE_FORCE_NEWLINE_IF_EOF state
		 */
		if (t.kind == EOF)
		{
	...
	void SkipLexicalActions(Token matchedToken)
	{
		switch (jjmatchedKind)
		{
			...
			case 8:
				if (jjmatchedPos == -1)
				{
					if (jjbeenHere[3] && jjemptyLineNo[3] == input_stream.getBeginLine() && jjemptyColNo[3] == input_stream.getBeginColumn())
			...
			case 12:
				...
				// or indentetion expected (if stdprompt == true, ovveride last
				// cond)
