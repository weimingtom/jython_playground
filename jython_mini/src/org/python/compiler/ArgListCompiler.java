...
public class ArgListCompiler extends Visitor implements PythonGrammarTreeConstants
{
	...
	public void visitArgs(argumentsType args) throws Exception
	{
		for (int i = 0; i < args.args.length; i++)
		{
			...
			{
				Assign ass = new Assign(new exprType[] { args.args[i] }, new Name(name, Name.Load, args.args[i]), args.args[i]);
		...
		{
			if (defaults[i] == null)
				throw new ParseException("non-default argument follows default argument", args.args[args.args.length - defaults.length + i]);
		}
	}
