...
public class CodeCompiler extends Visitor implements ClassConstants
{
	 ...
	 * is not needed because a new CodeCompiler is used for each PyCode, ie.
	 * each 'function'. When returning through finally's all the
	...
	public Object visitFunctionDef(FunctionDef node) throws Exception
	{
		...
		scope.setup_closure();
		scope.dump();
		module.PyCode(new Suite(node.body, node), name, true, className, false, false, node.beginLine, scope, cflags).get(code);
		getDocString(node.body);
		if (!makeClosure(scope))
		{
			if (mrefs.PyFunction_init == 0)
			{
				mrefs.PyFunction_init = code.pool.Methodref("org/python/core/PyFunction", "<init>", "(" + $pyObj + $pyObjArr + $pyCode + $pyObj + ")V");
			}
		...
		else
		{
			...
			{
				mrefs.PyFunction_closure_init = code.pool.Methodref("org/python/core/PyFunction", "<init>", "(" + $pyObj + $pyObjArr + $pyCode + $pyObj + $pyObjArr + ")V");
			}
	...
	public Object visitRaise(Raise node) throws Exception
	{
		...
		else if (node.inst == null)
		{
			...
			{
				mrefs.makeException1 = code.pool.Methodref("org/python/core/Py", "makeException", "(" + $pyObj + ")" + $pyExc);
			}
		...
		else if (node.tback == null)
		{
			...
			{
				mrefs.makeException2 = code.pool.Methodref("org/python/core/Py", "makeException", "(" + $pyObj + $pyObj + ")" + $pyExc);
			}
		...
		else
		{
			...
			{
				mrefs.makeException3 = code.pool.Methodref("org/python/core/Py", "makeException", "(" + $pyObj + $pyObj + $pyObj + ")" + $pyExc);
			}
	...
	public Object visitImport(Import node) throws Exception
	{
		...
		for (int i = 0; i < node.names.length; i++)
		{
			...
			if (node.names[i].asname != null)
			{
				...
				{
					mrefs.importOneAs = code.pool.Methodref("org/python/core/imp", "importOneAs", "(" + $str + $pyFrame + ")" + $pyObj);
				}
			...
			else
			{
				...
				{
					mrefs.importOne = code.pool.Methodref("org/python/core/imp", "importOne", "(" + $str + $pyFrame + ")" + $pyObj);
				}
	...
	public Object visitImportFrom(ImportFrom node) throws Exception
	{
		...
		if (node.names.length > 0)
		{
			...
			{
				mrefs.importFrom = code.pool.Methodref("org/python/core/imp", "importFrom", "(" + $str + $strArr + $pyFrame + ")" + $pyObjArr);
			}
	...
	public void exceptionTest(int exc, Label end_of_exceptions, TryExcept node, int index) throws Exception
	{
		for (int i = 0; i < node.handlers.length; i++)
		{
			...
			if (handler.type != null)
			{
				...
				{
					mrefs.match_exception = code.pool.Methodref("org/python/core/Py", "matchException", "(" + $pyExc + $pyObj + ")Z");
				}
	...
	public Object visitTryFinally(TryFinally node) throws Exception
	{
		...
		{
			mrefs.add_traceback = code.pool.Methodref("org/python/core/Py", "addTraceback", "(" + $throwable + $pyFrame + ")V");
		}
	...
	public Object visitTryExcept(TryExcept node) throws Exception
	{
		...
		{
			mrefs.set_exception = code.pool.Methodref("org/python/core/Py", "setException", "(" + $throwable + $pyFrame + ")" + $pyExc);
		}
	...
	static String[] bin_methods = new String[] { 
		null, "_add", "_sub", "_mul", "_div", 
		"_mod", "_pow", "_lshift", "_rshift", 
		"_or", "_xor", "_and", "_floordiv", 
	};
	...
	static String[] aug_methods = new String[] { 
		null, "__iadd__", "__isub__", "__imul__", 
		"__idiv__", "__imod__", "__ipow__", 
		"__ilshift__", "__irshift__", "__ior__", 
		"__ixor__", "__iand__", "__ifloordiv__", 
	};
	...
	public Object Invoke(Attribute node, SimpleNode[] values) throws Exception
	{
		...
		switch (values.length)
		{
			case 0:
				if (mrefs.invokea0 == 0)
				{
					mrefs.invokea0 = code.pool.Methodref("org/python/core/PyObject", "invoke", "(" + $str + ")" + $pyObj);
				}
			...
			case 1:
				if (mrefs.invokea1 == 0)
				{
					mrefs.invokea1 = code.pool.Methodref("org/python/core/PyObject", "invoke", "(" + $str + $pyObj + ")" + $pyObj);
				}
				visit(values[0]);
			...
			case 2:
				if (mrefs.invokea2 == 0)
				{
					mrefs.invokea2 = code.pool.Methodref("org/python/core/PyObject", "invoke", "(" + $str + $pyObj + $pyObj + ")" + $pyObj);
				}
				visit(values[0]);
				visit(values[1]);
			...
			default:
				...
				if (mrefs.invoke2 == 0)
				{
					mrefs.invoke2 = code.pool.Methodref("org/python/core/PyObject", "invoke", "(" + $str + $pyObjArr + ")" + $pyObj);
				}
	...
	public Object visitCall(Call node) throws Exception
	{
		...
		if (node.starargs != null || node.kwargs != null)
		{
			...
			{
				mrefs.callextra = code.pool.Methodref("org/python/core/PyObject", "_callextra", "(" + $pyObjArr + $strArr + $pyObj + $pyObj + ")" + $pyObj);
			}
		...
		else if (keys.length > 0)
		{
			...
			if (mrefs.call1 == 0)
			{
				mrefs.call1 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObjArr + $strArr + ")" + $pyObj);
			}
		...
		else
		{
			switch (values.length)
			{
				...
				case 1:
					if (mrefs.calla1 == 0)
					{
						mrefs.calla1 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObj + ")" + $pyObj);
					}
					visit(values[0]);
				...
				case 2:
					if (mrefs.calla2 == 0)
					{
						mrefs.calla2 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObj + $pyObj + ")" + $pyObj);
					}
					visit(values[0]);
					visit(values[1]);
				...
				case 3:
					if (mrefs.calla3 == 0)
					{
						mrefs.calla3 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObj + $pyObj + $pyObj + ")" + $pyObj);
					}
					visit(values[0]);
					visit(values[1]);
					visit(values[2]);
				...
				case 4:
					if (mrefs.calla4 == 0)
					{
						mrefs.calla4 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObj + $pyObj + $pyObj + $pyObj + ")" + $pyObj);
					}
					visit(values[0]);
					visit(values[1]);
					visit(values[2]);
				...
				default:
					...
					if (mrefs.call2 == 0)
					{
						mrefs.call2 = code.pool.Methodref("org/python/core/PyObject", "__call__", "(" + $pyObjArr + ")" + $pyObj);
					}
	...
	public Object Slice(Subscript node, Slice slice) throws Exception
	{
		...
		switch (ctx)
		{
			case Subscript.Del:
				if (mrefs.delslice == 0)
				{
					mrefs.delslice = code.pool.Methodref("org/python/core/PyObject", "__delslice__", "(" + $pyObj + $pyObj + $pyObj + ")V");
				}
			...
			case Subscript.Load:
				if (mrefs.getslice == 0)
				{
					mrefs.getslice = code.pool.Methodref("org/python/core/PyObject", "__getslice__", "(" + $pyObj + $pyObj + $pyObj + ")" + $pyObj);
				}
			...
			case Subscript.Store:
				...
				if (mrefs.setslice == 0)
				{
					mrefs.setslice = code.pool.Methodref("org/python/core/PyObject", "__setslice__", "(" + $pyObj + $pyObj + $pyObj + $pyObj + ")V");
				}
	...
	public Object visitSubscript(Subscript node) throws Exception
	{
		...
		switch (ctx)
		{
			...
			case Subscript.Load:
				if (mrefs.getitem == 0)
				{
					mrefs.getitem = code.pool.Methodref("org/python/core/PyObject", "__getitem__", "(" + $pyObj + ")" + $pyObj);
				}
			...
			case Subscript.Store:
				...
				if (mrefs.setitem == 0)
				{
					mrefs.setitem = code.pool.Methodref("org/python/core/PyObject", "__setitem__", "(" + $pyObj + $pyObj + ")V");
				}
	...
	public Object visitAttribute(Attribute node) throws Exception
	{
		...
		switch (ctx)
		{
			...
			case Attribute.Store:
				...
				if (mrefs.setattr == 0)
				{
					mrefs.setattr = code.pool.Methodref("org/python/core/PyObject", "__setattr__", "(" + $str + $pyObj + ")V");
				}
	...
	public Object seqSet(exprType[] nodes) throws Exception
	{
		...
		{
			mrefs.unpackSequence = code.pool.Methodref("org/python/core/Py", "unpackSequence", "(" + $pyObj + "I)" + $pyObjArr);
		}
		code.aload(temporary);
	...
	{
		/*
		 * if (mode ==AUGSET) throw new ParseException(
	...
	/*
	 * public Object fplist(SimpleNode node) throws Exception { if (mode == SET)
	 * return seqSet(node); throw new ParseException("in fplist node", node); }
	 */
	...
	public Object visitListComp(ListComp node) throws Exception
	{
		...
		stmtType n = new Expr(new Call(new Name(tmp_append, Name.Load, node), new exprType[] { node.elt }, new keywordType[0], null, null, node), node);
	...
	public Object visitDict(Dict node) throws Exception
	{
		...
		{
			mrefs.PyDictionary_init = code.pool.Methodref("org/python/core/PyDictionary", "<init>", "(" + $pyObjArr + ")V");
		}
	...
	public Object visitLambda(Lambda node) throws Exception
	{
		...
		if (!makeClosure(scope))
		{
			...
			{
				mrefs.PyFunction_init1 = code.pool.Methodref("org/python/core/PyFunction", "<init>", "(" + $pyObj + $pyObjArr + $pyCode + ")V");
			}
		...
		else
		{
			...
			{
				mrefs.PyFunction_closure_init1 = code.pool.Methodref("org/python/core/PyFunction", "<init>", "(" + $pyObj + $pyObjArr + $pyCode + $pyObjArr + ")V");
			}
	...
	public Object visitSlice(Slice node) throws Exception
	{
		...
		{
			mrefs.PySlice_init = code.pool.Methodref("org/python/core/PySlice", "<init>", "(" + $pyObj + $pyObj + $pyObj + ")V");
		}
	...
	public Object visitClassDef(ClassDef node) throws Exception
	{
		...
		module.PyCode(new Suite(node.body, node), name, false, name, true, false, node.beginLine, scope, cflags).get(code);
		...
		if (!makeClosure(scope))
		{
			...
			{
				mrefs.makeClass = code.pool.Methodref("org/python/core/Py", "makeClass", "(" + $str + $pyObjArr + $pyCode + $pyObj + ")" + $pyObj);
			}
		...
		else
		{
			...
			{
				mrefs.makeClass_closure = code.pool.Methodref("org/python/core/Py", "makeClass", "(" + $str + $pyObjArr + $pyCode + $pyObj + $pyObjArr + ")" + $pyObj);
			}
		...
		switch (ctx)
		{
			case Name.Load:
				...
				if (syminf != null)
				{
					...
					if ((flags & ScopeInfo.GLOBAL) != 0 || optimizeGlobals && (flags & (ScopeInfo.BOUND | ScopeInfo.CELL | ScopeInfo.FREE)) == 0)
					{
						emitGetGlobal(name);
						return null;
					}
					if (fast_locals)
					{
						...
						if ((flags & ScopeInfo.BOUND) != 0)
						{
							...
							{
								mrefs.getlocal2 = code.pool.Methodref("org/python/core/PyFrame", "getlocal", "(I)" + $pyObj);
							}
				...
				{
					mrefs.getlocal1 = code.pool.Methodref("org/python/core/PyFrame", "getname", "(" + $str + ")" + $pyObj);
				}
			...
			case Name.Store:
				...
				else
				{
					if (!fast_locals)
					{
						...
						{
							mrefs.setlocal1 = code.pool.Methodref("org/python/core/PyFrame", "setlocal", "(" + $str + $pyObj + ")V");
						}
					...
					else
					{
						...
						if ((syminf.flags & ScopeInfo.CELL) != 0)
						{
							...
							if (mrefs.setderef == 0)
							{
								mrefs.setderef = code.pool.Methodref("org/python/core/PyFrame", "setderef", "(I" + $pyObj + ")V");
							}
						...
						else
						{
							...
							{
								mrefs.setlocal2 = code.pool.Methodref("org/python/core/PyFrame", "setlocal", "(I" + $pyObj + ")V");
							}
			...
			case Name.Del:
			{
				...
				if (syminf != null && (syminf.flags & ScopeInfo.GLOBAL) != 0)
				{
					...
					{
						mrefs.delglobal = code.pool.Methodref("org/python/core/PyFrame", "delglobal", "(" + $str + ")V");
					}
				...
				else
				{
					if (!fast_locals)
					{
						...
						{
							mrefs.dellocal1 = code.pool.Methodref("org/python/core/PyFrame", "dellocal", "(" + $str + ")V");
						}
					...
					else
					{
						...
						if ((syminf.flags & ScopeInfo.CELL) != 0)
						{
							module.error("can not delete variable '" + name + "' referenced in nested scope", true, node);
						}
						code.iconst(syminf.locals_index);
	 ...
	 * the inlined finally of a parent try:finally: or the reentry block after a
	 * yield. Thus we keep around a set of exception ranges that the catch block
	...
	class ExceptionHandler
	{
		...
		public void addExceptionHandlers(Label handlerStart) throws Exception
		{
			...
			for (int i = 0; i < exceptionStarts.size(); ++i)
			{
				...
				{
					code.addExceptionHandler((Label) exceptionStarts.elementAt(i), (Label) exceptionEnds.elementAt(i), handlerStart, throwable);
				}
			}
		}
	}
