...
public class ScopesCompiler extends Visitor implements ScopeConstants
{
	...
	/*
	 * private static void illassign(SimpleNode node) throws Exception { String
	 * target = "operator"; if (node.id ==
	 * PythonGrammarTreeConstants.JJTCALL_OP) { target = "function call"; } else
	 * if ((node.id == PythonGrammarTreeConstants.JJTFOR_STMT)) { target =
	 * "list comprehension"; } throw new
	 * ParseException("can't assign to "+target,node); }
	 */
