...
public class ProxyMaker implements ClassConstants
{
	...
	public void callMethod(Code code, String name, Class[] parameters, Class ret, Class[] exceptions) throws Exception
	{
		...
		switch (getType(ret))
		{
			...
			default:
				...
				code.invokevirtual(jcall);
				/*
				 * catching exceptions is not vm mandatory Label forname_start
				 * =code.getLabel(); Label forname_end = code.getLabel(); Label
				 * forname_exch_start = code.getLabel(); Label forname_exch_end
				 * = code.getLabel(); forname_start.setPosition();
				 */
				int forname = code.pool.Methodref("java/lang/Class", "forName", "(" + $str + ")" + $clss);
				code.ldc(ret.getName());
				code.invokestatic(forname);
				/*
				 * forname_end.setPosition(); code.goto_(forname_exch_end);
				 * forname_exch_start.setPosition(); code.stack = 1; // never
				 * reached, but this code keeps the verifier happy code.pop();
				 * code.aconst_null(); code.dup();
	...
	public void addMethod(Method method, int access) throws Exception
	{
		...
		if (!isAbstract)
		{
			...
			int jfindattr = code.pool.Methodref("org/python/core/Py", "jfindattr", "(" + $pyProxy + $str + ")" + $pyObj);
		...
		else
		{
			if (!isAdapter)
			{
				int jgetattr = code.pool.Methodref("org/python/core/Py", "jgetattr", "(" + $pyProxy + $str + ")" + $pyObj);
	...
	public void addSuperMethod(String methodName, String superName, String declClass, Class[] parameters, Class ret, String sig, int access) throws Exception
	{
		...
		{
			/*
			 * rationale: JC java-class, P proxy-class subclassing JC in order
			 * to avoid infinite recursion P should define super__foo only if no
			 * class between P and JC in the hierarchy defines it yet; this
			 * means that the python class needing P is the first that redefines
			 * the JC method foo.
			 */
			try
			{
